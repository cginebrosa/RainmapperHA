    ...
    def clean_and_convert(self, dict_list: list):
        ...
        for dict in dict_list:
            ...
            for key, value in dict.items():
                ...
                #MONTHLY
                if key in ('High','Low','Avg'):
                    converted_dict[key] = self.temperature(value)
                if key in ('High_1','Low_1','Avg_1'):
                    converted_dict[key] = self.dew_point(value)
                if key in ('High_2','Low_2','Avg_2'):
                    converted_dict[key] = self.humidity(value)
                if key in ('High_3','Low_3','Avg_3'):
                    converted_dict[key] = self.speed(value)
                if key in ('High_4','Low_4'):
                    converted_dict[key] = self.pressure(value)
                if key in ('Sum'):
                    converted_dict[key] = self.precipitation(value)

                # Station header data
                if key == 'StationID':
                    converted_dict['StationID'] = value
                if key == 'StationName':
                    converted_dict['StationName'] = value
                if key == 'Comarca':
                    converted_dict['Comarca'] = value
                if key == 'Municipi':
                    converted_dict['Municipi'] = value
                if key == 'Provincia':
                    converted_dict['Provincia'] = value
                if key == 'Elevation':
                    converted_dict['Elevation'] = value
                if key == 'Latitude':
                    converted_dict['Latitude'] = value
                if key == 'Longitude':
                    converted_dict['Longitude'] = value
