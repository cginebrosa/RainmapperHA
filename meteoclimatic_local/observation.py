...
class Observation:
    ...
    def from_feed_item(cls, feed_item):
        ...
        station_geolat = str(f'{float(feed_item.lat.text):.2f}')        # Added Get latitude
        station_geolon = str(f'{float(feed_item.long.text):.2f}')       # Added Get longitude
        if station_geolat < station_geolon:                              # Exchange lat/long if they seems  to be flipped
            station_geolat, station_geolon = station_geolon, station_geolat
        station = Station(station_name, station_code, station_url, station_geolat, station_geolon) # Added latitude&longitude
