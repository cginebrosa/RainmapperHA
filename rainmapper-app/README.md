...
`maplibre_observations_mobile_enabled` permite mostrar el modo observaciones en
teléfonos y está desactivado por defecto. Es un ajuste general del complemento;
además cada usuario necesita el permiso **Observation map access** en Usuarios,
también desactivado por defecto. En local se configura en
`rainmapper-local/options.local-ha-ui.json`. Cambiar este ajuste requiere reiniciar
HA local o el complemento; no requiere reiniciar el worker.
...
`wunderground_monthly_api` usa por defecto la API de observaciones diarias de Wunderground para refrescar desde el dia 1 hasta hoy; durante los dias 1--7 refresca tambien el mes anterior completo. `wunderground_weekly_api` conserva como alternativa una ventana movil de siete dias; los dos modos son mutuamente excluyentes. `wunderground_encoding_order` elige una de las seis permutaciones validas de `gzip`, `identity` y `deflate`, con `gzip,identity,deflate` como valor predeterminado. Si una consulta que incluye hoy devuelve una observacion con mas de cuatro horas de antiguedad, Rainmapper prueba las variantes restantes en ese orden, conserva la respuesta con timestamp mas reciente y muestra los reintentos, recuperaciones y respuestas que siguen antiguas en el resumen. Si el modo API seleccionado falla, Rainmapper registra el fallback, usa el scraper HTML existente y muestra `API fallback errors`. Los backfills siguen solicitando sus fechas locales explicitas.
