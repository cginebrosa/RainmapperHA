...
def observation_map_controls_html(*, gis: bool = False) -> str:
    controls = """
    ...
    """
    if gis:
        controls = controls.replace('<div class="observation-layer-panel"', '<button class="observation-map-control observation-gis-toggle" type="button" aria-pressed="false" title="Consultar GIS / DEM del punto"><span>GIS</span><span>DEM</span></button><div class="observation-layer-panel"')
    return controls
    ...
    <div id="{html.escape(modal_id, quote=True)}" class="modal-layer">
      <div class="modal-card evidence-map-modal">
        <div class="modal-header">
          ...
          {photo_html}
          <aside class="observation-map-gis-panel" hidden aria-label="GIS / DEM del punto" aria-live="polite"></aside>
        ...
        <div class="observation-site-map-shell">
          ...
          {observation_map_controls_html(gis=True)}
    ...
    <div id="{html.escape(modal_id, quote=True)}" class="modal-layer">
      ...
      <form class="modal-card modal-card-wide observation-form" method="post" action=""{form_enctype}>
        ...
        <div class="observation-form-footer">
          ...
          {map_action}
          {mushroom_observation_gis_ui.controls(row)}
...
def observation_site_map_assets() -> str:
    ...
    (()=>{
      const initialize=()=>document.querySelectorAll('.modal-layer:target .observation-site-map:not([data-ready])').forEach(node=>{
        ...
        map.on('load',()=>{const bounds=new maplibregl.LngLatBounds();const features=(data.known_sites||[]).map(site=>({type:'Feature',properties:{kind:site.kind,id:site.id,name:site.name,area_id:site.area_id,assigned:site.id===data.assigned_micro_area_id},geometry:site.geometry}));map.addSource('known-sites',{type:'geojson',data:{type:'FeatureCollection',features}});map.addLayer({id:'known-areas-fill',type:'fill',source:'known-sites',filter:['==',['get','kind'],'area'],paint:{'fill-color':'#e9aa3b','fill-opacity':.10}});map.addLayer({id:'known-areas-line',type:'line',source:'known-sites',filter:['==',['get','kind'],'area'],paint:{'line-color':'#e9aa3b','line-width':2,'line-dasharray':[3,2]}});map.addLayer({id:'known-micro-fill',type:'fill',source:'known-sites',filter:['==',['get','kind'],'micro_area'],paint:{'fill-color':['case',['get','assigned'],'#35dcff','#16b8f4'],'fill-opacity':['case',['get','assigned'],.34,.18]}});map.addLayer({id:'known-micro-line',type:'line',source:'known-sites',filter:['==',['get','kind'],'micro_area'],paint:{'line-color':['case',['get','assigned'],'#9af1ff','#16b8f4'],'line-width':['case',['get','assigned'],4,2]}});
          ...
          const gisInspector=window.rainmapperObservationMapGIS?.attach(map,node,first,()=>!coordinateEditing&&siteMode==='select'&&!draw);
          ...
          const markers=new Map();map.rainmapperObservationMarkers=markers;observations.forEach((item,index)=>{bounds.extend([item.lon,item.lat]);const marker=new maplibregl.Marker({color:'#ef4444'}).setLngLat([item.lon,item.lat]).addTo(map),element=marker.getElement();element.classList.add('observation-default-marker');element.classList.toggle('selected',index===0);element.title=item.observation_id;markers.set(item.observation_id,marker);element.addEventListener('click',event=>{if(gisInspector?.enabled){event.stopPropagation();popup.remove();gisInspector.select(marker.getLngLat());return;}if(coordinateEditing)return;event.stopPropagation();selectMappedObservation(node.dataset.observationSiteMap,item.observation_id);const lngLat=marker.getLngLat();showPopup(lngLat,map.project(lngLat),item);});});if(observations.length>1)map.fitBounds(bounds,{padding:70,maxZoom:16});else map.jumpTo({center:[first.lon,first.lat],zoom:15});
          ...
          ['known-micro-fill','known-areas-fill'].forEach(layer=>{map.on('mouseenter',layer,()=>{if(!coordinateEditing)map.getCanvas().style.cursor='pointer';});map.on('mouseleave',layer,()=>{if(!coordinateEditing)map.getCanvas().style.cursor='';});});map.on('click',event=>{if(gisInspector?.enabled){popup.remove();gisInspector.select(event.lngLat);return;}if(coordinateEditing&&coordinateForm&&coordinateForm.rainmapperSelectCoordinate){popup.remove();coordinateForm.rainmapperSelectCoordinate(event.lngLat);return;}showPopup(event.lngLat,event.point);});});
      });
    ...
    """ + mushroom_observation_gis_ui.map_assets()
