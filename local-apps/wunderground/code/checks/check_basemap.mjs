...
try{
  ...
  await evaluate('document.getElementById("basemap-toggle").click()');
  assert.equal(await evaluate('document.getElementById("basemap-panel").hidden'),false);
  assert.equal(await evaluate('document.querySelectorAll("#basemap select").length'),0);
  assert.deepEqual(await evaluate('Array.from(document.querySelectorAll("#basemap label"),x=>x.textContent)'),['Satélite+','Híbrido','Topográfico','Liberty']);
  assert.equal(await evaluate('getComputedStyle(document.getElementById("basemap")).backgroundColor'),'rgb(255, 255, 255)');
  assert.equal(await evaluate('document.querySelector("#basemap input:checked").value'),'esri-satellite-vector');
  await evaluate(`document.querySelector('#basemap input[value="esri-hybrid"]').click()`);
  await until('ready && !!map.getSource("stations") && map.isSourceLoaded("stations")');
  assert.equal(await evaluate('activeBasemap'),'esri-hybrid');
  assert.equal(await evaluate('document.getElementById("basemap-panel").hidden'),true);
  await evaluate('document.getElementById("basemap-toggle").click()');
  assert.equal(await evaluate('document.querySelector("#basemap input:checked").value'),'esri-hybrid');
  const shot=await send('Page.captureScreenshot',{format:'png'});
  await fs.writeFile(path.join(root,'..','basemap-list-verified.png'),Buffer.from(shot.data,'base64'));
  assert.equal(exceptions.length,0,JSON.stringify(exceptions));
  console.log(JSON.stringify({status:'passed',directOptions:4,whiteBackground:true,selectionWorks:true,activeMarked:true}));
