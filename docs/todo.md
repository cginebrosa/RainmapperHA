...
`0.2.101` queda validada manualmente en HA: la WebUI `Users` refresca y busca correctamente sin refrescar el navegador. `0.2.103` arreglo que la WebUI pasara `create_aemet` a jobs `update/all`, pero en HA fallo la publicacion experimental AEMET al reconstruir Tomap por un `merge` sobre columnas opcionales con tipos pandas distintos. `0.2.104` queda publicada en GHCR con ese fix (`sha256:530e69cce6cbbcb01a5a33d6e66a9621a4ef0a2e67a2693d682e4cb3afc36b24`); siguiente paso recomendado: instalar/validar `0.2.104` en HA con `create_aemet=true`, `aemet_api_key` configurada y `estacions_aemet.csv` ya copiado a `/share/rainmapper/Data`.
...
- [ ] Observar prueba externa con usuarios reales
  ...
  - Estado: prueba en curso. HA ejecuta `Run all` con schedule `01:45 - 05:00 - 08:00 - 11:00 - 14:00 - 17:00 - 20:00 - 23:55`. `0.2.101` quedo validada manualmente para WebUI `Users`; `0.2.104` queda publicada con AEMET opcional y pendiente de instalar/validar en HA.
