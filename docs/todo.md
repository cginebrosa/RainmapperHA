...
- [ ] Llevar perfiles de setas a HA y crear mantenimiento WebUI
  ...
  - Estado: backend minimo implementado en `rainmapper_core/mushroom_store.py` y endpoints admin en `web_server.py`: validar, exportar, plantilla vacia e importar/reemplazar perfiles/catalogos. Primera UI WebUI de catalogos implementada en `/mushrooms/catalogs` con hub, busqueda/filtros, creacion de entradas por grupo con plantilla minima validada, detalle JSON editable, validacion cruzada e import/export/plantilla del catalogo completo. Publicado en HA `0.2.150` con smoke local OK; pendiente de validacion visual/operativa en HA. Pendiente posterior: UI de perfiles y documentar uso en `rainmapper-app/DOCS.md`.
