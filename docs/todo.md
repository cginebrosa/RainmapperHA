...
`0.2.125` es la version HA actual en `rainmapper-app/config.yaml`, `rainmapper-app/Dockerfile`, cache-busters y changelog, y esta publicada en GHCR como `0.2.125/latest` con digest multi-arch `sha256:685823800b18918ccea3e315d2f66eb333e6eb10ec854f3df83a941442aaa29d`. Corrige la ocultacion real de botones heatmap/metrica para usuarios no admin, anade pagina WebUI `/log` sin auto-refresh, refuerza la cabecera sticky del historial de popups MapLibre y usa escala dinamica min/max para metricas no lluvia segun periodo/filtros/fuentes seleccionadas, permitiendo temperaturas negativas. Tambien conserva el fix `0.2.122` de lectura de incrementales grandes con inferencia completa para evitar los `DtypeWarning` de pandas vistos en `rainmapper.py` y `tomap.py`. `0.2.124` queda como version HA publicada con digest multi-arch `sha256:9586b5c682bc1b170798d927c7abbdbbaa7269fe9204da06baa5b9b758d4cbbc`. Auditoria GitHub del 2026-06-24: el repo remoto esta publico (`private=false`, `visibility=public`) para que HA detecte updates. Siguiente paso recomendado: instalar/actualizar `0.2.125` en HA, validar admin vs usuario no-admin, escala dinamica de metricas, cabecera del historial y log externo; despues volver a poner el repo privado y limpiar GHCR remoto conservando solo la version validada/latest y sus auxiliares multi-arch.
...
- [ ] Explorar capas MapLibre por metrica y heatmap
  ...
  - Estado: experimento publicado en `0.2.120` y ampliado hasta `0.2.125`: se anadio una variante publica `/local/rainmapper-maplibre-heatmap/index.html` generada por el publicador HA con `experimentalHeatmap: true`. El codigo usa una fuente GeoJSON separada para el heatmap (`stations-heatmap`) con todas las estaciones validas del periodo filtradas por fuente activa, y otra fuente para puntos filtrados por lluvia minima/fuente. Tras validar visualmente `0.2.123`, `0.2.124` promociono la funcionalidad al visor protegido con restriccion UI completa. `0.2.125` corrige que los botones ocultos se mostrasen por CSS en usuarios no admin y ajusta la escala de metricas no lluvia a min/max real de las estaciones filtradas del periodo seleccionado; lluvia conserva la escala robusta existente. La persistencia backend por `device_id` de metrica, heatmap activo/inactivo, opacidad, radio, intensidad y curva de peso solo aplica al visor protegido autenticado con usuario `admin`; la ruta experimental publica muestra controles para pruebas pero no guarda preferencias en `/auth/device-settings`. Pendiente validar en HA que un usuario free/basic/pro no ve ningun control de heatmap y que la escala dinamica resulta legible.
