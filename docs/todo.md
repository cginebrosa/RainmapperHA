...
Nota de continuidad 2026-06-28: version HA actual `0.2.170` publicada para validar en HA. Imagen `ghcr.io/cginebrosa/rainmapperha:0.2.170` y `latest`, digest multi-arch `sha256:ee00db78e9b1fcc5663043899f89f77c04f992132ecfff97f94a602f5883096c`. `0.2.169` quedo publicada con imagen `ghcr.io/cginebrosa/rainmapperha:0.2.169`, digest multi-arch `sha256:0d1198c9d715144fe8022e79de6224342a5a635980498c46f5105f7581d3dc06`, pero no debe cerrarse como buena porque los modales de `New species`, `Duplicate species` y `Restore species` se veian en HA pero no permitian escribir ni pulsar botones: el backdrop heredaba `z-index` de los modales antiguos de Users y capturaba los clics por encima del panel. `0.2.170` corrige esa capa. `0.2.169` avanza el mantenimiento de especies: toolbar superior por encima del titulo, titulo alineado con tabs de seccion, `Trophic mode` en la misma linea de `Ecology`, modales server-rendered para `New species`, `Duplicate species`, `Archive species` y `Restore species`, duplicado funcional como draft, archivado/restauracion funcional y borrado permanente solo desde el archivo. Decision defensiva: una especie activa no se borra directamente; primero debe archivarse, y solo una especie archivada puede eliminarse definitivamente con doble advertencia. La UI de especies sigue en iteracion hacia `docs/mushrooms/ui/profiles/mushroom-profiles-species.png`. `web_server.py` conserva rutas, POST, persistencia y orquestacion de validacion; las pantallas grandes de setas viven en `rainmapper-app/app/mushroom_profiles_ui.py` y `rainmapper-app/app/mushroom_catalogs_ui.py`. El modulo comun de validacion es `rainmapper_core/mushroom_validation.py` y lo usan tanto la WebUI HA como `scripts/validate-mushroom-data.py`. `docs/mushrooms/ui/reference-catalogs/reference-catalog-domain-impact-reference.png` sigue pendiente como pantalla informativa/analitica futura, no bloquea el flujo principal de mantenimiento.
...
Validar `0.2.170` en HA. Foco principal: abrir `/mushrooms/profiles` y confirmar que los modales `New species`, `Duplicate species` y `Restore species` permiten escribir en campos y pulsar botones. Despues, comprobar que la toolbar queda arriba, que el titulo queda alineado con `Summary/Species/Observations/...`, que `Trophic mode` queda en la linea de `Ecology` y que las listas de afinidades empiezan mas arriba. Probar modales: `New species` crea draft y abre la especie; `Duplicate species` propone nuevo ID/nombre y abre la copia; `Archive species` exige escribir el ID y quita la especie activa; `Restore species` muestra lista de archivadas, permite restaurar y permite `Delete permanently` solo desde archivo con doble advertencia. Revalidar guardado normal de una especie desde `Metadata`, scoring decimal/validacion, raw JSON, import/export y navegacion hacia `/mushrooms/catalogs`.

- [ ] Validar en HA el flujo `Create species`, `Duplicate species`, `Archive species`, `Restore species` y `Delete permanently`
  - Contexto: `0.2.169` implementa la primera version funcional de ciclo de vida de especies con estrategia defensiva. Una especie activa solo puede archivarse; el borrado permanente solo aparece para especies archivadas.
  - Criterio de aceptacion futuro: `Create species` crea una especie draft validada y la abre; `Duplicate species` clona desde la especie actual con nuevo ID/nombre y reset de metadata/calibracion a draft/no calibrada; `Archive species` exige escribir el ID, crea backup y mueve el perfil completo a `archived/mushroom_profiles_archived.json`; `Restore species` devuelve el perfil si el ID activo esta libre; `Delete permanently` elimina solo del archivo con doble advertencia.
  - Estado: implementado en `0.2.169`, pendiente de validacion visual/operativa en HA.
...
- [x] Implementar archivado/restauracion de especies
  ...
  - Estado: implementado en `0.2.169` con archivado previo obligatorio, restauracion desde archivo y borrado permanente solo para especies archivadas con doble advertencia; pendiente de validacion HA.
