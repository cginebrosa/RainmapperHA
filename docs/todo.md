...
Nota de continuidad 2026-06-28: version HA actual `0.2.164` preparada para publicar/validar. Mejora la capa funcional y visual de mantenimiento de especies: `0.2.161` anade validacion semantica compartida para bloquear duplicados en arrays simples, IDs duplicados en afinidades y solapes entre `phenology.main_months` y `phenology.secondary_months`; `0.2.162` hace visibles esos bloqueos como alertas de `Validation error`; `0.2.163` anade alta guiada `New species` con plantilla completa en estado `draft`, `not_calibrated` y `requires_human_validation=true`; `0.2.164` inicia el redisenyo visual hacia `docs/mushrooms/ui/profiles/mushroom-profiles-species.png` y extrae helpers de render de especies a `rainmapper-app/app/mushroom_profiles_ui.py`. El modulo comun de validacion es `rainmapper_core/mushroom_validation.py` y lo usan tanto la WebUI HA como `scripts/validate-mushroom-data.py`. `docs/mushrooms/ui/reference-catalogs/reference-catalog-domain-impact-reference.png` sigue pendiente como pantalla informativa/analitica futura, no bloquea el flujo principal de mantenimiento.
...
Validar `0.2.164` en HA. Foco visual: comprobar que `/mushrooms/profiles` se acerca al mockup `mushroom-profiles-species.png`: listado izquierdo compacto, cabecera de especie con chips, tab `General` tipo dashboard con tarjetas, meses como chips, barras de scoring y barra de acciones inferior. Foco funcional: crear una especie de prueba desde `New species`, confirmar que aparece seleccionada, que queda como `draft`/`not_calibrated`, que se puede editar después desde los tabs, y que un ID duplicado se bloquea con alerta visible. Comprobar también que el validador sigue mostrando `0 errors · 7 warnings` con los datos actuales tras borrar o revertir la especie de prueba si se creó sólo para validación. La extracción de render de especies ya se ha iniciado en `rainmapper-app/app/mushroom_profiles_ui.py`; si la validación visual es buena, seguir sacando catalogos/perfiles de `web_server.py` de forma acotada, sin cambiar comportamiento.
