...
`0.2.133` es la version HA actual en `rainmapper-app/config.yaml`, `rainmapper-app/Dockerfile`, cache-busters y changelog. Esta publicada en GHCR como `0.2.133/latest` con digest multi-arch `sha256:d0b6432a123b718a5ad58ed990b48a7847589b8e74d4487ea1010787d2a775f2` y queda pendiente de instalar/validar visualmente en HA. Incluye permisos MapLibre por usuario para `Heatmap access` y `Metric selector access`, defaults activos al crear usuarios `admin`, y documenta la deuda futura de permisos por perfil/tipo de usuario con overrides por usuario. Commit de release `b9e69cf` pusheado a `inicial`; repo GitHub abierto temporalmente para deteccion HA (`private=false`, `visibility=public`). `0.2.132` queda como ultima version validada visualmente en HA por el usuario el 2026-06-25, publicada con digest `sha256:801e77ff582afe64d47dd7f56935e424732ee687ab98b5436afaf789c49762ad`. Siguiente paso recomendado: instalar `0.2.133` en HA, validar que los permisos por usuario ocultan/muestran correctamente Heatmap y selector de metricas, y despues volver a poner el repo GitHub privado y limpiar GHCR remoto conservando solo la version validada/latest y auxiliares multi-arch.
