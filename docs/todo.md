...
Nota historica supersedida 2026-06-27: esta nota describia `0.2.150` como version actual y un fix local pendiente. Ese estado ya no es vigente. Se conserva solo como antecedente del primer hub de catalogos; el estado operativo verificado en el cierre actual es `0.2.176` segun las notas de continuidad anteriores.

## Proximo paso recomendado
Validar visualmente en HA `0.2.176`, especialmente `/mushrooms/catalogs`: tarjetas superiores de grupos con nombres humanos desde `mushroom_labels.json`, sin IDs raw largos para los catalogos de observaciones, y ausencia de `missing label:` en grupos actuales. Revalidar tambien `/mushrooms/profiles?section=observations`: alta con Google Maps URL, alta con latitud/longitud, edicion, archivado/restauracion/borrado permanente defensivo, orden/color de abundancia/use, y que `Parameters` sigue mostrando labels humanas. Mantener pendiente `docs/mushrooms/ui/reference-catalogs/reference-catalog-domain-impact-reference.png` como pantalla informativa futura.
...
- [ ] Hacer configurable el idioma de los mantenimientos de setas
  - Prioridad: baja/media.
  - Contexto: `rainmapper-app/config.yaml` ya expone `ui_language` con valores `en`, `es` y `ca`, pero las pantallas de setas siguen renderizando principalmente en ingles y solo usan `mushroom_labels.json` de forma parcial.
  - Criterio de aceptacion futuro: leer `ui_language` desde opciones HA, aplicar el idioma a labels de parametros, observaciones, species y reference catalogs, y mantener fallback explicito a `en` solo dentro del diccionario de labels/catalogos. Mas adelante extender el mismo patron al control board y users.
  - Estado: tarea futura; no bloquear la primera base real de observaciones.
