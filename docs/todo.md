...
Nota de continuidad 2026-06-27: version HA actual `0.2.158` publicada en GHCR y pendiente de validacion en HA. Imagen `ghcr.io/cginebrosa/rainmapperha:0.2.158/latest` con digest multi-arch `sha256:e1ed912d11dc47a7a748173218dfef4c1907ba4ff784daca78b447cb69872cf6`. Incluye la primera WebUI de mantenimiento de especies en `/mushrooms/profiles`: listado de especies, buscador, editor guiado del perfil, selects de ecologia respaldados por catalogos, guardado validado con backup y panel avanzado JSON/import-export/plantilla. `0.2.157` queda publicada con validaciones cruzadas visibles para catalogos. `docs/mushrooms/ui/reference-catalogs/reference-catalog-domain-impact-reference.png` sigue pendiente como pantalla informativa/analitica futura, no bloquea el flujo principal de mantenimiento.
...
Validar `0.2.158` en HA. Foco: boton `Mushroom species` desde Control Panel, acceso directo `/mushrooms/profiles`, seleccion de especies, buscador, selects de ecologia con etiquetas de catalogo, guardado de un campo inocuo con backup/validacion, panel avanzado JSON plegado, import/export/plantilla de perfiles y que `Mushroom catalogs` siga funcionando como en `0.2.157`. Despues de validarlo, decidir si se mejora la UI de perfiles v1 o si se avanza hacia observaciones/calibracion. La pantalla `reference-catalog-domain-impact-reference.png` queda como mejora informativa posterior.
