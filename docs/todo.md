...
Validar visualmente y funcionalmente en HA `0.2.175`, especialmente `/mushrooms/profiles?section=observations`: seeding de `mushroom_labels.json` y `mushroom_observations.json`, alta con Google Maps URL, alta con latitud/longitud, edicion, archivado/restauracion/borrado permanente defensivo, orden/color de abundancia/use, y que `Parameters` sigue mostrando labels humanas. Mantener pendiente `docs/mushrooms/ui/reference-catalogs/reference-catalog-domain-impact-reference.png` como pantalla informativa futura.
