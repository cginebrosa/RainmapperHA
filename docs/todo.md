...
Nota de continuidad 2026-06-28: version HA actual `0.2.162` preparada para publicar/validar. Mejora la capa funcional de mantenimiento de especies: `0.2.161` anade validacion semantica compartida para bloquear duplicados en arrays simples, IDs duplicados en afinidades y solapes entre `phenology.main_months` y `phenology.secondary_months`; `0.2.162` hace visibles esos bloqueos como alertas de `Validation error` y redirige al aviso tras un guardado fallido. El modulo comun es `rainmapper_core/mushroom_validation.py` y lo usan tanto la WebUI HA como `scripts/validate-mushroom-data.py`. `docs/mushrooms/ui/reference-catalogs/reference-catalog-domain-impact-reference.png` sigue pendiente como pantalla informativa/analitica futura, no bloquea el flujo principal de mantenimiento.
...
Validar `0.2.162` en HA. Foco: intentar guardar una especie con el mismo mes en `Main months` y `Secondary months`, confirmar que se bloquea con una alerta visible `Validation error`, comprobar que duplicados de afinidades siguen bloqueados o filtrados, y verificar que el validador sigue mostrando `0 errors · 7 warnings` con los datos actuales. Siguiente mejora funcional pendiente: anadir flujo guiado `New species`; ahora mismo no hay boton de alta de especie y la unica forma seria importacion/edicion JSON completa, que no debe considerarse el flujo operativo recomendado. Si se valida, antes de seguir ampliando mucho la UI de setas conviene extraer mas pantallas/helpers de setas fuera de `web_server.py` hacia modulos propios para evitar que el servidor HA principal siga creciendo; hacerlo como refactor acotado, sin cambiar comportamiento.
