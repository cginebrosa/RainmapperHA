...
`0.2.147` esta publicada en GHCR, pusheada a GitHub y pendiente de validacion en HA. Incluye el rediseño compacto del Control Panel de Rainmapper como dashboard con tabs internos, manteniendo todas las acciones existentes (`Run update`, `Generate maps`, `Run all`, `App settings`, `Users`, `Update only` por fuente, abrir visores, abrir mapas, abrir log, `Disable all` y `Enable all`). Validar especialmente dentro de Home Assistant/ingress: tabs Summary/Data sources/Viewers/Maps/Logs/Errors, quick actions, tablas compactas de fuentes, enlaces de visores/mapas/log, y que no se pierde ninguna accion respecto al panel anterior. `0.2.146` queda validada/dada por buena en HA para la WebUI Users redisenada. Seguir observando el incremental AEMET/backfill reciente.
