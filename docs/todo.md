...
`0.2.135` es la version HA actual en `rainmapper-app/config.yaml`, `rainmapper-app/Dockerfile`, cache-busters y changelog. Esta publicada en GHCR como `0.2.135/latest` con digest multi-arch `sha256:39bb92bc38daf32251525ae2aab1e69f3f90914906bf8f78f230321abfc424e2`, commit de release `d341bb2` y queda pendiente de validar visualmente en HA por el usuario. Anade configuracion HA para activar hover de estaciones desde `maplibre_hover_zoom=6`, defaults HA de heatmap (`soft`, `65`, `90`, `70`) y boton `Reset heatmap defaults` en Settings. Estado operativo: repo GitHub abierto temporalmente (`private=false`, `visibility=public`, rama `inicial`) para que Home Assistant detecte el update. Siguiente paso recomendado: instalar/validar `0.2.135` en HA; si queda validada, volver a poner el repo privado y limpiar GHCR remoto conservando solo `0.2.135/latest` y sus auxiliares multi-arch. `0.2.134` queda como ultimo rollback validado previo, con digest `sha256:d012482068a869fe98feda19f9a8c919f4958a8e1a8a52fa674df17bbaec8c57`.
...
- [x] Ajustar umbral de hover MapLibre
  ...
  - Estado: publicado inicialmente en `0.2.95` con umbral `7`; `0.2.135` lo hace configurable desde HA como `maplibre_hover_zoom` con default `6`. Pendiente de validar en HA/movil y retirar el indicador temporal si ya no hace falta.
...
- [ ] Validar zoom visible temporal MapLibre
  ...
  - Estado: publicado en `0.2.95` con umbral `7`; `0.2.135` lo hace configurable desde HA como `maplibre_hover_zoom` con default `6`. Retirar el indicador al fijar el umbral definitivo.
