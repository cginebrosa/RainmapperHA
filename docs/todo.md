...
`0.2.129` es la version HA actual en `rainmapper-app/config.yaml`, `rainmapper-app/Dockerfile`, cache-busters y changelog, y esta publicada en GHCR como `0.2.129/latest` con digest multi-arch `sha256:fecfd9b74a85fc76f1e5fe15b5246fdb947325901dfcdaa19602ab5e3335daee`. Incluye la persistencia backend de settings Heatmap de `0.2.127` y restaura el comportamiento correcto del historial de popups MapLibre: el scroll usa toda la ventana del popup, la cabecera de estacion y el summary desaparecen al subir, y solo la fila de columnas queda sticky arriba. Conserva la semantica de valores ausentes de `0.2.126`: `null`/vacío/no numerico es sin dato, no cero; los puntos sin dato se pintan gris con radio minimo y quedan fuera del heatmap; los ceros numericos reales siguen contando como cero. Tomap preserva `Total` ausente en grupos sin lluvia numerica, evitando fabricar `0` por `groupby().sum()`. Tambien conserva el fix `0.2.122` de lectura de incrementales grandes con inferencia completa para evitar los `DtypeWarning` de pandas vistos en `rainmapper.py` y `tomap.py`. `0.2.128` queda publicada con digest multi-arch `sha256:3a9cda363caede04c836624de97cd77433e1b0dbd50f397290e1734cc69db220` y `0.2.126` queda como rollback inmediato con digest multi-arch `sha256:518efa5e02fcbe61b0949a4788687e165c203c3effa7603583c393706e9ab8c3`. Auditoria GitHub del 2026-06-24: el repo remoto esta publico (`private=false`, `visibility=public`) para que HA detecte updates. Siguiente paso recomendado: instalar/actualizar `0.2.129` en HA, validar persistencia Heatmap admin, scroll del historial en popup largo, escalas/heatmap con lluvia/humedad/temperatura/viento y confirmar que usuario no-admin sigue sin controles heatmap/metrica; despues volver a poner el repo privado y limpiar GHCR remoto conservando solo la version validada/latest y sus auxiliares multi-arch.
