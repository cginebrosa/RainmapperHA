...
Nota de continuidad 2026-06-27: version HA actual `0.2.159` publicada en GHCR y pendiente de validacion en HA. Imagen `ghcr.io/cginebrosa/rainmapperha:0.2.159/latest` con digest multi-arch `sha256:1be57f1b923e5d6fa6e4cd53ea6729ffba1fa6674184cfcc85fd6ba111e7366b`. Mejora la WebUI de mantenimiento de especies de `0.2.158`: tabs internas, calibracion como area propia, bloqueo de duplicados dentro de afinidades y navegacion bidireccional entre perfiles y catalogos. `docs/mushrooms/ui/reference-catalogs/reference-catalog-domain-impact-reference.png` sigue pendiente como pantalla informativa/analitica futura, no bloquea el flujo principal de mantenimiento.
...
Validar `0.2.159` en HA. Foco: tabs del mantenimiento de especies, pestaña `Calibration`, guardado normal, bloqueo de duplicados en afinidades, boton desde catalogos hacia especies y que `Mushroom catalogs` siga funcionando. Si se valida, antes de seguir ampliando mucho la UI de setas conviene extraer las pantallas/helpers de setas fuera de `web_server.py` hacia un modulo propio para evitar que el servidor HA principal siga creciendo; hacerlo como refactor acotado, sin cambiar comportamiento.
