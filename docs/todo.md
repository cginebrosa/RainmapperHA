...
Nota de continuidad 2026-06-28: version HA actual `0.2.163` preparada para publicar/validar. Mejora la capa funcional de mantenimiento de especies: `0.2.161` anade validacion semantica compartida para bloquear duplicados en arrays simples, IDs duplicados en afinidades y solapes entre `phenology.main_months` y `phenology.secondary_months`; `0.2.162` hace visibles esos bloqueos como alertas de `Validation error`; `0.2.163` anade alta guiada `New species` con plantilla completa en estado `draft`, `not_calibrated` y `requires_human_validation=true`. El modulo comun es `rainmapper_core/mushroom_validation.py` y lo usan tanto la WebUI HA como `scripts/validate-mushroom-data.py`. `docs/mushrooms/ui/reference-catalogs/reference-catalog-domain-impact-reference.png` sigue pendiente como pantalla informativa/analitica futura, no bloquea el flujo principal de mantenimiento.
