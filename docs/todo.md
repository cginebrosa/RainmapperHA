...
Nota de continuidad 2026-06-28: version HA actual `0.2.171` publicada para validar en HA. Imagen `ghcr.io/cginebrosa/rainmapperha:0.2.171` y `latest`, digest multi-arch `sha256:d018069ece91716ea5d526c22a6c57c08652598d5f09e761c19a0c5f4a8edf1c`. `0.2.170` quedo publicada con imagen `ghcr.io/cginebrosa/rainmapperha:0.2.170`, digest multi-arch `sha256:ee00db78e9b1fcc5663043899f89f77c04f992132ecfff97f94a602f5883096c`, y corrigio los modales de `New species`, `Duplicate species` y `Restore species` que en `0.2.169` se veian pero no permitian escribir ni pulsar botones. `0.2.171` ajusta `Archive species`: muestra el `species_id` seleccionado como solo lectura, no obliga a reescribirlo, archiva con el boton del modal y vuelve a la vista base/lista de especies. Tambien ordena visualmente la lista lateral de especies por `scientific_name` sin reordenar el JSON persistido. Decision defensiva: una especie activa no se borra directamente; primero debe archivarse, y solo una especie archivada puede eliminarse definitivamente con doble advertencia. La UI de especies sigue en iteracion hacia `docs/mushrooms/ui/profiles/mushroom-profiles-species.png`. `web_server.py` conserva rutas, POST, persistencia y orquestacion de validacion; las pantallas grandes de setas viven en `rainmapper-app/app/mushroom_profiles_ui.py` y `rainmapper-app/app/mushroom_catalogs_ui.py`. El modulo comun de validacion es `rainmapper_core/mushroom_validation.py` y lo usan tanto la WebUI HA como `scripts/validate-mushroom-data.py`. `docs/mushrooms/ui/reference-catalogs/reference-catalog-domain-impact-reference.png` sigue pendiente como pantalla informativa/analitica futura, no bloquea el flujo principal de mantenimiento.
...
Validar `0.2.171` en HA. Foco principal: abrir `/mushrooms/profiles`, confirmar que la lista lateral queda ordenada alfabeticamente por nombre cientifico y probar `Archive species`: debe mostrar el `species_id` seleccionado como solo lectura, no pedir reescribirlo, archivar al pulsar el boton y volver a la vista base/lista de especies. Revalidar que `New species`, `Duplicate species` y `Restore species` aceptan foco, escritura y submit; `Restore species` debe mostrar lista de archivadas, permitir restaurar y permitir `Delete permanently` solo desde archivo con doble advertencia. Revalidar guardado normal de una especie desde `Metadata`, scoring decimal/validacion, raw JSON, import/export y navegacion hacia `/mushrooms/catalogs`.
