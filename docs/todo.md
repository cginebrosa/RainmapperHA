...
`0.2.126` es la version HA actual en `rainmapper-app/config.yaml`, `rainmapper-app/Dockerfile`, cache-busters y changelog, y esta publicada en GHCR como `0.2.126/latest` con digest multi-arch `sha256:518efa5e02fcbe61b0949a4788687e165c203c3effa7603583c393706e9ab8c3`. Corrige la semantica de valores ausentes en MapLibre y Tomap: `null`/vacío/no numerico es sin dato, no cero; los puntos sin dato se pintan gris con radio minimo y quedan fuera del heatmap; los ceros numericos reales siguen contando como cero. Tomap preserva `Total` ausente en grupos sin lluvia numerica, evitando fabricar `0` por `groupby().sum()`. Tambien conserva el fix `0.2.122` de lectura de incrementales grandes con inferencia completa para evitar los `DtypeWarning` de pandas vistos en `rainmapper.py` y `tomap.py`. `0.2.125` queda como version HA publicada con digest multi-arch `sha256:685823800b18918ccea3e315d2f66eb333e6eb10ec854f3df83a941442aaa29d`. Auditoria GitHub del 2026-06-24: el repo remoto esta publico (`private=false`, `visibility=public`) para que HA detecte updates. Siguiente paso recomendado: instalar/actualizar `0.2.126` en HA, validar escalas/heatmap con lluvia, humedad, temperatura y viento, confirmar que usuario no-admin sigue sin controles heatmap/metrica; despues volver a poner el repo privado y limpiar GHCR remoto conservando solo la version validada/latest y sus auxiliares multi-arch.
...
- [ ] Explorar capas MapLibre por metrica y heatmap
  ...
  - Estado: experimento publicado en `0.2.120` y ampliado hasta `0.2.126`: se anadio una variante publica `/local/rainmapper-maplibre-heatmap/index.html` generada por el publicador HA con `experimentalHeatmap: true`. El codigo usa una fuente GeoJSON separada para el heatmap (`stations-heatmap`) con todas las estaciones validas del periodo filtradas por fuente activa, y otra fuente para puntos filtrados por lluvia minima/fuente. Tras validar visualmente `0.2.123`, `0.2.124` promociono la funcionalidad al visor protegido con restriccion UI completa. `0.2.125` corrige que los botones ocultos se mostrasen por CSS en usuarios no admin y ajusta la escala de metricas no lluvia a min/max real de las estaciones filtradas del periodo seleccionado; `0.2.126` hace que todas las metricas, incluida lluvia, traten valores ausentes como sin dato, no como cero, y excluye esas estaciones del heatmap de la metrica activa. La persistencia backend por `device_id` de metrica, heatmap activo/inactivo, opacidad, radio, intensidad y curva de peso solo aplica al visor protegido autenticado con usuario `admin`; la ruta experimental publica muestra controles para pruebas pero no guarda preferencias en `/auth/device-settings`. Pendiente validar en HA que un usuario free/basic/pro no ve ningun control de heatmap y que la escala/heatmap no se contaminan por `null`.
