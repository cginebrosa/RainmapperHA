...
`0.2.128` es la version HA actual en `rainmapper-app/config.yaml`, `rainmapper-app/Dockerfile`, cache-busters y changelog, y esta publicada en GHCR como `0.2.128/latest` con digest multi-arch `sha256:3a9cda363caede04c836624de97cd77433e1b0dbd50f397290e1734cc69db220`. Incluye la persistencia backend de settings Heatmap de `0.2.127` y corrige la cabecera del historial de popups MapLibre para que quede pegada arriba al hacer scroll en fichas con muchos registros. Conserva la semantica de valores ausentes de `0.2.126`: `null`/vacío/no numerico es sin dato, no cero; los puntos sin dato se pintan gris con radio minimo y quedan fuera del heatmap; los ceros numericos reales siguen contando como cero. Tomap preserva `Total` ausente en grupos sin lluvia numerica, evitando fabricar `0` por `groupby().sum()`. Tambien conserva el fix `0.2.122` de lectura de incrementales grandes con inferencia completa para evitar los `DtypeWarning` de pandas vistos en `rainmapper.py` y `tomap.py`. `0.2.127` queda publicada con digest multi-arch `sha256:588a31ae93e217c64d41b492743a653d602bb89732e7f75f173ccdba3461a649` y `0.2.126` queda como rollback inmediato con digest multi-arch `sha256:518efa5e02fcbe61b0949a4788687e165c203c3effa7603583c393706e9ab8c3`. Auditoria GitHub del 2026-06-24: el repo remoto esta publico (`private=false`, `visibility=public`) para que HA detecte updates. Siguiente paso recomendado: instalar/actualizar `0.2.128` en HA, validar persistencia Heatmap admin, cabecera sticky de historial en popup largo, escalas/heatmap con lluvia/humedad/temperatura/viento y confirmar que usuario no-admin sigue sin controles heatmap/metrica; despues volver a poner el repo privado y limpiar GHCR remoto conservando solo la version validada/latest y sus auxiliares multi-arch.
