...
Nota de continuidad 2026-06-28: version HA actual `0.2.172` publicada, instalada y validada correctamente en HA por el usuario. Imagen `ghcr.io/cginebrosa/rainmapperha:0.2.172` y `latest`, digest multi-arch `sha256:acc20d71b82257e4c55736a5cbabbb935002ffd8b6c69b076040f222bfbcecd4`. `0.2.171` quedo publicada con imagen `ghcr.io/cginebrosa/rainmapperha:0.2.171`, digest multi-arch `sha256:d018069ece91716ea5d526c22a6c57c08652598d5f09e761c19a0c5f4a8edf1c`, ajusto `Archive species` para mostrar el `species_id` como solo lectura y ordeno visualmente la lista lateral de especies por `scientific_name`. `0.2.172` corrige la tarjeta `Weather model summary` del tab `General/Summary` de especies para leer las claves documentadas del `weather_model` real (`rain_7d_min_mm`, `temp_min_7d_optimal_min_c`, etc.), igual que el tab `Weather`. Decision defensiva: una especie activa no se borra directamente; primero debe archivarse, y solo una especie archivada puede eliminarse definitivamente con doble advertencia. La UI de especies sigue en iteracion hacia `docs/mushrooms/ui/profiles/mushroom-profiles-species.png`. `web_server.py` conserva rutas, POST, persistencia y orquestacion de validacion; las pantallas grandes de setas viven en `rainmapper-app/app/mushroom_profiles_ui.py` y `rainmapper-app/app/mushroom_catalogs_ui.py`. El modulo comun de validacion es `rainmapper_core/mushroom_validation.py` y lo usan tanto la WebUI HA como `scripts/validate-mushroom-data.py`. `docs/mushrooms/ui/reference-catalogs/reference-catalog-domain-impact-reference.png` sigue pendiente como pantalla informativa/analitica futura, no bloquea el flujo principal de mantenimiento.
...
Validar visualmente en HA `0.2.174`, especialmente el layout compacto de `Parameters`, labels humanas, hosts primarios/secundarios y guardado de parametros. Siguiente mejora funcional probable: definir el modelo/store de observaciones reales, importacion JSON/CSV con preview y validacion, porque esa pantalla sera la base para calibrar especies contra observaciones positivas y negativas. Mantener pendiente `docs/mushrooms/ui/reference-catalogs/reference-catalog-domain-impact-reference.png` como pantalla informativa futura.
...
- [ ] Crear mantenimiento visual de etiquetas de parametros de setas
  - Prioridad: baja.
  - Contexto: `mushroom-data/mushroom_parameter_labels.json` centraliza nombres humanos de parametros en `en`, `es` y `ca` para que la UI no muestre claves raw como `rain_7d_min_mm`.
