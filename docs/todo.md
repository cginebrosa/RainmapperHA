...
Nota de continuidad 2026-06-27: version HA actual `0.2.160` publicada en GHCR y pendiente de validacion en HA. Mejora la WebUI de mantenimiento de especies de `0.2.159`: metricas superiores compactas, los desplegables de afinidades ocultan IDs ya usados en el mismo bloque para evitar duplicados antes de guardar, y se mantiene el bloqueo backend para JSON/raw. `docs/mushrooms/ui/reference-catalogs/reference-catalog-domain-impact-reference.png` sigue pendiente como pantalla informativa/analitica futura, no bloquea el flujo principal de mantenimiento.
...
Validar `0.2.160` en HA. Foco: barra compacta de metricas superiores, desplegables de afinidades sin IDs ya usados, bloqueo backend si se fuerza un duplicado por JSON/raw, tabs/calibracion y que `Mushroom catalogs` siga funcionando. Si se valida, antes de seguir ampliando mucho la UI de setas conviene extraer las pantallas/helpers de setas fuera de `web_server.py` hacia un modulo propio para evitar que el servidor HA principal siga creciendo; hacerlo como refactor acotado, sin cambiar comportamiento.
