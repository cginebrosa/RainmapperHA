...
`0.2.137` es la version HA actual en `rainmapper-app/config.yaml`, `rainmapper-app/Dockerfile`, cache-busters y changelog. Esta publicada en GHCR como `0.2.137/latest` con digest multi-arch `sha256:539c879d2c7f9dfc282d671b71c627a858b48d59778e3195ec2d0254accee928`, commit de release `1bcd681`, validada/dada por buena en HA el 2026-06-25 y cerrada operacionalmente. Corrige la incidencia detectada en `0.2.136`: los defaults HA del heatmap se aplicaban bien a dispositivos nuevos, pero al cambiar `config.yaml` y reiniciar la app, el boton `Reset heatmap defaults` podia seguir usando un `config.js` protegido cacheado por el navegador. `0.2.137` sirve `/protected/maplibre/config.js` con `Cache-Control: no-store, max-age=0` para que los cambios de defaults HA se recojan tras reiniciar la app. Estado operativo: repo GitHub privado (`private=true`, `visibility=private`, rama `inicial`) y GHCR remoto limpio conservando solo `0.2.137/latest` y cuatro auxiliares multi-arch sin tag del mismo push. Siguiente paso recomendado: continuar con el backfill manual AEMET cuando el diario publique los dias pendientes. `0.2.134` queda como ultimo rollback visual validado previo; `0.2.135` y `0.2.136` no deben cerrarse como versiones buenas para defaults/reset heatmap.
