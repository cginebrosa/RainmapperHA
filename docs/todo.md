...
`0.2.148` esta publicada en GHCR, pusheada a GitHub y pendiente de validacion en HA. Incluye el ajuste compacto del panel expandido de User Management: `User details` en una fila, permisos en grid de tarjetas preparado para crecer, auditoria compacta dentro del formulario de guardado y bloque `Security` separado en una fila mas densa, conservando backend, endpoints y handlers actuales. Validar especialmente dentro de Home Assistant/ingress: guardar usuario, permisos Heatmap/Metric selector/IDW, auditoria, `Set password`, `Reset password`, `Delete user`, `Delete device`, `Delete all devices`, Devices y busqueda/accordion. `0.2.147` parece funcionar bien en HA con el Control Panel redisenado. Seguir observando el incremental AEMET/backfill reciente.
