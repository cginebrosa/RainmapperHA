...
Nota historica supersedida 2026-06-27: esta nota describia `0.2.150` como version actual y un fix local pendiente. Ese estado ya no es vigente. Se conserva solo como antecedente del primer hub de catalogos; el estado operativo verificado en el cierre actual es `0.2.177` segun las notas de continuidad anteriores.

## Proximo paso recomendado
Validar visualmente en HA `0.2.177`, especialmente el selector `ui_language` del add-on (`en`, `es`, `ca`) tras reinicio. Revisar `/mushrooms/profiles`: tabs `Summary`, `Species`, `Observations`, `Parameters` y `Calibration` sin mezcla incoherente de idiomas ni `missing label`; `Observations` con alta por Google Maps URL, alta por latitud/longitud, edicion, archivado/restauracion/borrado permanente defensivo, orden/color de abundancia/use; `/mushrooms/catalogs` con tarjetas de grupos humanas y conteo de referencias desde observaciones, de modo que valores usados como `experienced` no aparezcan unused. Mantener pendiente `docs/mushrooms/ui/reference-catalogs/reference-catalog-domain-impact-reference.png` como pantalla informativa futura.
...
- [x] Hacer configurable el idioma de los mantenimientos de setas
  ...
  - Estado: implementado en `0.2.177` para el dominio `mushrooms`. Queda como tarea futura extender el mismo patron al Control Board y Users.
...
- [ ] Extender `ui_language` a Control Board y Users
  ...
  - Estado: tarea futura; no bloquear validacion de `0.2.177`.
