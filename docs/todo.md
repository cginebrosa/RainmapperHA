...
`0.2.130` es la version HA actual en `rainmapper-app/config.yaml`, `rainmapper-app/Dockerfile`, cache-busters y changelog, y esta publicada en GHCR como `0.2.130/latest` con digest multi-arch `sha256:b9dbd0a7a40b4b16293d073e174ab7db5a8551152470b032b7bfb2276e3014a8`. Incluye la persistencia backend de settings Heatmap de `0.2.127`, restaura el comportamiento correcto del historial de popups MapLibre de `0.2.129` y ajusta la fila sticky de columnas para quedar pegada al borde superior del popup sin dejar ver filas por encima. Tambien prioriza el arranque de fuentes con 3 hilos para lanzar primero `AEMET`, `Meteoclimatic` y `Meteocat`, dejando `Wunderground` como cuarta tarea. Conserva la semantica de valores ausentes de `0.2.126`: `null`/vacío/no numerico es sin dato, no cero; los puntos sin dato se pintan gris con radio minimo y quedan fuera del heatmap; los ceros numericos reales siguen contando como cero. Tomap preserva `Total` ausente en grupos sin lluvia numerica, evitando fabricar `0` por `groupby().sum()`. `0.2.129` queda publicada con digest multi-arch `sha256:fecfd9b74a85fc76f1e5fe15b5246fdb947325901dfcdaa19602ab5e3335daee` y `0.2.126` queda como rollback inmediato con digest multi-arch `sha256:518efa5e02fcbe61b0949a4788687e165c203c3effa7603583c393706e9ab8c3`. Auditoria GitHub del 2026-06-24: el repo remoto esta publico (`private=false`, `visibility=public`) para que HA detecte updates. Siguiente paso recomendado: instalar/actualizar `0.2.130` en HA, validar persistencia Heatmap admin, sticky del historial en popup largo, orden de arranque de fuentes en log y confirmar que usuario no-admin sigue sin controles heatmap/metrica; despues volver a poner el repo privado y limpiar GHCR remoto conservando solo la version validada/latest y sus auxiliares multi-arch.
