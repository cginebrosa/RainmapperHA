...
`0.2.124` es la version HA actual en `rainmapper-app/config.yaml`, `rainmapper-app/Dockerfile`, cache-busters y changelog, y esta publicada en GHCR como `0.2.124/latest` con digest multi-arch `sha256:9586b5c682bc1b170798d927c7abbdbbaa7269fe9204da06baa5b9b758d4cbbc`. Promociona los controles de heatmap al visor protegido solo para usuarios `admin`: boton `HM`, boton rapido de metrica, pestaña `Heatmap` en Settings y persistencia backend por `device_id` de metrica, activo/inactivo, opacidad, radio, intensidad y curva de peso. Los usuarios no admin del visor protegido no ven controles de heatmap/metrica y no pueden activar heatmap por query string; la ruta publica experimental `/local/rainmapper-maplibre-heatmap/index.html` sigue mostrando controles para pruebas, sin persistencia backend. Tambien conserva el fix `0.2.122` de lectura de incrementales grandes con inferencia completa para evitar los `DtypeWarning` de pandas vistos en `rainmapper.py` y `tomap.py`. `0.2.123` queda como version HA publicada con digest multi-arch `sha256:89118fb88892ef2910eea09b8e72c3c460040d0d24d39b73801cd82d2a81a590`. Auditoria GitHub del 2026-06-24: el repo remoto esta publico (`private=false`, `visibility=public`) para que HA detecte updates. Siguiente paso recomendado: instalar/actualizar `0.2.124` en HA, validar admin vs usuario no-admin y confirmar que desaparece el warning de pandas; despues volver a poner el repo privado y limpiar GHCR remoto conservando solo la version validada/latest y sus auxiliares multi-arch.
...
- [ ] Explorar capas MapLibre por metrica y heatmap
  ...
  - Estado: experimento publicado en `0.2.120` y ampliado hasta `0.2.124`: se anadio una variante publica `/local/rainmapper-maplibre-heatmap/index.html` generada por el publicador HA con `experimentalHeatmap: true`. El codigo usa una fuente GeoJSON separada para el heatmap (`stations-heatmap`) con todas las estaciones validas del periodo filtradas por fuente activa, y otra fuente para puntos filtrados por lluvia minima/fuente. Tras validar visualmente `0.2.123`, `0.2.124` promociona la funcionalidad al visor protegido con restriccion UI completa: boton heatmap, boton rapido de metricas y pestana/seccion `Heatmap` solo aparecen para usuarios `admin` o en la ruta experimental; usuarios no admin vuelven a lluvia normal y no pueden activar heatmap por query string. La persistencia backend por `device_id` de metrica, heatmap activo/inactivo, opacidad, radio, intensidad y curva de peso solo aplica al visor protegido autenticado con usuario `admin`; la ruta experimental publica muestra controles para pruebas pero no guarda preferencias en `/auth/device-settings`. Pendiente validar en HA que un usuario free/basic/pro no ve ningun control de heatmap.
