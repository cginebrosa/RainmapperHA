...
`0.2.137` es la version HA actual en `rainmapper-app/config.yaml`, `rainmapper-app/Dockerfile`, cache-busters y changelog. Esta publicada en GHCR como `0.2.137/latest` con digest multi-arch `sha256:539c879d2c7f9dfc282d671b71c627a858b48d59778e3195ec2d0254accee928`, commit de release `1bcd681` y queda pendiente de validar en HA. Corrige la incidencia detectada en `0.2.136`: los defaults HA del heatmap se aplicaban bien a dispositivos nuevos, pero al cambiar `config.yaml` y reiniciar la app, el boton `Reset heatmap defaults` podia seguir usando un `config.js` protegido cacheado por el navegador. `0.2.137` sirve `/protected/maplibre/config.js` con `Cache-Control: no-store, max-age=0` para que los cambios de defaults HA se recojan tras reiniciar la app. Estado operativo: repo GitHub abierto temporalmente (`private=false`, `visibility=public`, rama `inicial`) para que Home Assistant detecte el update. Siguiente paso recomendado: instalar/validar `0.2.137` en HA; cambiar defaults de heatmap en configuracion HA, reiniciar la app y comprobar que `Reset heatmap defaults` restaura los nuevos valores sin limpiar cache manualmente. Si queda validada, volver a poner el repo privado y limpiar GHCR remoto conservando solo `0.2.137/latest` y auxiliares multi-arch. `0.2.134` queda como ultimo rollback validado previo; `0.2.135` y `0.2.136` no deben cerrarse como versiones buenas para defaults/reset heatmap.
