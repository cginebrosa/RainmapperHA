...
`0.2.119` es la version HA actual en `rainmapper-app/config.yaml`, `rainmapper-app/Dockerfile`, cache-busters y changelog, y esta publicada en GHCR como `0.2.119/latest` con digest multi-arch `sha256:d6220a7ce7b186b7c598cbadadcb2f11c3d3bf41de3b33c5272dfcf2d993fe95`. Incluye: alineacion de temperatura, humedad y viento en el historial MapLibre mediante subcolumnas internas sin normalizar con ceros; logging AEMET con timestamp/contexto para distinguir `429` en endpoint indice vs URL `datos` mas pausa de 1s entre ambas peticiones; contadores persistentes AEMET `429` en ultimas 24h/runs consecutivos visibles en rojo en la WebUI cuando son distintos de cero; y botones `Update only` por fuente en la WebUI para lanzar un update temporal de una sola fuente sin modificar opciones HA ni reiniciar/schedule. `0.2.118` queda como version HA publicada y validada visualmente por el usuario el 2026-06-24, con digest multi-arch GHCR `sha256:07ce37c45de5f705aeb1621f4fb680a7b2c9360014ee1ccbb95322e7815d0e96`. `0.2.117` se conserva en GHCR como rollback con digest `sha256:e12749d4b16a48c362f731eb4f03dbb850b71988061602396c51293ad0350d65`. Auditoria GitHub del 2026-06-24: el repo remoto esta publico (`private=false`, `visibility=public`) para que HA detectara el update. Siguiente paso recomendado: instalar/actualizar `0.2.119` en HA y validarla; despues volver a poner el repo privado y limpiar GHCR remoto conservando solo la version validada/latest y sus auxiliares multi-arch.
...
- [ ] Explorar capas MapLibre por metrica y heatmap
  ...
  - Estado: experimento local iniciado tras `0.2.119`: se anadio una variante publica `/local/rainmapper-maplibre-heatmap/index.html` generada por el publicador HA con `experimentalHeatmap: true`, sin cambiar el visor protegido por defecto. El codigo usa una fuente GeoJSON separada para el heatmap (`stations-heatmap`) con todas las estaciones validas del periodo, y otra fuente para puntos filtrados por lluvia minima/fuente. Pendiente validar visualmente, ajustar escala/colores/radio si hace falta y decidir si se publica/promociona.
