...
`0.2.120` es la version HA actual en `rainmapper-app/config.yaml`, `rainmapper-app/Dockerfile`, cache-busters y changelog, y esta publicada en GHCR como `0.2.120/latest` con digest multi-arch `sha256:9efce7f58351b4c1f3634cc7ce33e9ef389310544dc887caa199fd310f0ab2ad`. Incluye el experimento MapLibre heatmap en una ruta publica separada (`/local/rainmapper-maplibre-heatmap/index.html`) sin cambiar el visor protegido por defecto: boton `Heatmap`, selector y boton rapido de metrica (`Rain`, `Max temp`, `Min temp`, `Max humidity`, `Min humidity`, `Wind`), slider de opacidad, puntos/leyenda por metrica y heatmap con todas las estaciones validas del periodo. `0.2.119` queda como version HA publicada con digest multi-arch `sha256:d6220a7ce7b186b7c598cbadadcb2f11c3d3bf41de3b33c5272dfcf2d993fe95` e incluye alineacion de temperatura/humedad/viento en historial MapLibre, diagnostico/contadores AEMET `429` y botones `Update only` por fuente. `0.2.118` queda como version HA publicada y validada visualmente por el usuario el 2026-06-24, con digest multi-arch GHCR `sha256:07ce37c45de5f705aeb1621f4fb680a7b2c9360014ee1ccbb95322e7815d0e96`. Auditoria GitHub del 2026-06-24: el repo remoto esta publico (`private=false`, `visibility=public`) para que HA detecte updates. Siguiente paso recomendado: instalar/actualizar `0.2.120` en HA y validar visualmente el experimento heatmap; despues volver a poner el repo privado y limpiar GHCR remoto conservando solo la version validada/latest y sus auxiliares multi-arch.
...
- [ ] Llevar perfiles de setas a HA y crear mantenimiento WebUI
  - Contexto: `rainmapper_core/predictor/mushroom_profiles.json` contiene los perfiles parametricos iniciales de especies para el futuro predictor de floradas. El perfil base vive en core porque debe viajar con RainmapperHA y quedar disponible para cualquier runtime; mas adelante se podra mantener desde la webUI sin editar JSON a mano.
  - Ficheros relacionados: `rainmapper_core/predictor/mushroom_profiles.json`, `tests/test_predictor_profiles.py`, `rainmapper-app/app/web_server.py`, `rainmapper-app/DOCS.md`, futuro almacenamiento persistente en `/share/rainmapper`.
  - Criterio de aceptacion: incluir un perfil base en la imagen HA, copiarlo/sembrarlo en almacenamiento persistente si falta, crear una pagina WebUI de mantenimiento de especies que permita listar, editar y validar perfiles, proteger el guardado con el mismo schema/test de `tests/test_predictor_profiles.py`, y conservar backups o historial antes de sobrescribir cambios manuales. El archivo original `mushroom_profiles.json` debe conservarse como referencia; cualquier calibracion operativa debe escribirse en un archivo separado con sufijo `_calibrated` (`mushroom_profiles_calibrated.json`) y ese archivo calibrado sera el que use el predictor. La WebUI debe permitir ver/comparar valores originales y calibrados para recalibrar o detectar/restaurar errores.
...
- [x] Cerrar exposicion publica de repo, fallbacks y paquetes antiguos
  ...
  - Estado: completado para fallbacks/HTTPS/Cloudflare; repo privado queda como operacion pendiente al cierre de esta sesion porque la auditoria real del 2026-06-24 devuelve repo publico para update HA (`private=false`, `visibility=public`). HTTP->HTTPS activo, HSTS activo con `max-age=2592000; includeSubDomains`, `x-content-type-options: nosniff` presente, `router`, `leaflet` y `maplibre` redirigen a Cloudflare Access, y ruta protegida de datos en `rainmap` devuelve `401` sin sesion segun comprobaciones previas. GHCR conserva `0.2.120/latest`, `0.2.119`, `0.2.118` y `0.2.117` como rollback remoto hasta validar la nueva version. Tras validar `0.2.120` en HA, volver a poner el repo privado y limpiar GHCR remoto conservando solo esa version, `latest` y sus auxiliares multi-arch.
...
- [ ] Explorar capas MapLibre por metrica y heatmap
  ...
  - Criterio de aceptacion: validar primero en una ruta experimental no protegida, separada de los usuarios actuales, con un boton `Heatmap`, selector `Layer metric` (`Rain`, `Max temp`, `Min temp`, `Max humidity`, `Min humidity`, `Wind`), boton rapido de metrica similar al selector rapido de capas y slider de opacidad. Mantener el filtro existente de lluvia minima para los puntos, pero hacer que el heatmap use todas las estaciones validas del periodo. El selector de metrica debe afectar siempre a puntos y leyenda; el boton `Heatmap` solo anade o quita la capa de densidad. La UI debe dejar claro que el heatmap es una densidad ponderada por estaciones, no una interpolacion meteorologica exacta. Si se promociona al visor protegido de produccion, todas las preferencias nuevas (`heatmap` activo/inactivo, metrica seleccionada y opacidad) deben persistirse por `device_id` en `devices.json` igual que el resto de settings MapLibre, con defaults compatibles para dispositivos existentes.
  - Estado: experimento publicado en `0.2.120`: se anadio una variante publica `/local/rainmapper-maplibre-heatmap/index.html` generada por el publicador HA con `experimentalHeatmap: true`, sin cambiar el visor protegido por defecto. El codigo usa una fuente GeoJSON separada para el heatmap (`stations-heatmap`) con todas las estaciones validas del periodo, y otra fuente para puntos filtrados por lluvia minima/fuente. Pendiente validar visualmente en HA, ajustar escala/colores/radio si hace falta y decidir si se promociona al visor protegido con persistencia por dispositivo.
