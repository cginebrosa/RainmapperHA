...
`0.2.150` esta publicada en GHCR y pusheada a GitHub, con commit de release `ecf2ed8`, pero no debe validarse como buena: el enlace absoluto `/mushrooms/catalogs` falla bajo HA ingress y, al no entrar en la pantalla, no se crea `/share/rainmapper/mushroom-data/`. Siguiente paso recomendado: publicar un bump con el fix local ya aplicado y validar especialmente la nueva WebUI de catalogos: acceso desde Control Panel, siembra de defaults en `/share/rainmapper/mushroom-data/`, busqueda/filtros, detalle JSON editable, guardado con backup, creacion de entradas por grupo, validacion cruzada e import/export/plantilla del catalogo completo. Confirmar tambien que User Management sigue cerrado por defecto como en `0.2.149`. `0.2.149` queda publicada, instalada y validada/dada por buena en HA para User Management; `0.2.148` no debe cerrarse como buena por el bug visual de acordeon totalmente desplegado; `0.2.147` parece funcionar bien en HA con el Control Panel redisenado.
...
- [ ] Llevar perfiles de setas a HA y crear mantenimiento WebUI
  ...
  - Estado: backend minimo implementado en `rainmapper_core/mushroom_store.py` y endpoints admin en `web_server.py`: validar, exportar, plantilla vacia e importar/reemplazar perfiles/catalogos. Primera UI WebUI de catalogos implementada en `/mushrooms/catalogs` con hub, busqueda/filtros, creacion de entradas por grupo con plantilla minima validada, detalle JSON editable, validacion cruzada e import/export/plantilla del catalogo completo. Publicado en HA `0.2.150`, pero no cerrar como buena por 404 en HA ingress; fix local aplicado con rutas relativas, seeding al arranque y contador `Reference errors` basado en errores reales del validador, pendiente de bump/publicacion. Pendiente posterior: UI de perfiles y documentar uso en `rainmapper-app/DOCS.md`.

- [ ] Revisar validaciones cruzadas del mantenimiento de catalogos de setas
  ...
  - Estado: pendiente deliberado tras validar `0.2.156`; no cambiar datos automaticamente.
