...
`0.2.134` es la version HA actual en `rainmapper-app/config.yaml`, `rainmapper-app/Dockerfile`, cache-busters y changelog. Esta publicada en GHCR como `0.2.134/latest` con digest multi-arch `sha256:d012482068a869fe98feda19f9a8c919f4958a8e1a8a52fa674df17bbaec8c57`, commit de release `3f4c6b4` y queda validada visualmente en HA por el usuario el 2026-06-25. Corrige el espaciado de la cabecera sticky del historial MapLibre para que no tape el titulo `Ultims 30 registres` al desplegar y hacer scroll. Cierre operativo completado: repo GitHub privado (`private=true`, `visibility=private`, rama `inicial`) y GHCR remoto limpio tras borrar `0.2.133` y sus auxiliares; quedan solo `0.2.134`, `latest` y cuatro auxiliares multi-arch del mismo push. Siguiente paso recomendado: continuar con mejoras incrementales; no publicar nueva version ni abrir el repo salvo necesidad operativa concreta.
...
- [x] Ajustar umbral de hover MapLibre
  ...
  - Estado: publicado inicialmente en `0.2.95` con umbral `7`; cambio posterior local lo hace configurable desde HA como `maplibre_hover_zoom` con default `6`. Pendiente de validar en HA/movil y retirar el indicador temporal si ya no hace falta.
...
- [ ] Validar zoom visible temporal MapLibre
  ...
  - Estado: publicado en `0.2.95` con umbral `7`; cambio posterior local lo hace configurable desde HA como `maplibre_hover_zoom` con default `6`. Retirar el indicador al fijar el umbral definitivo.
