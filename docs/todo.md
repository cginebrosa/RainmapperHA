...
`0.2.132` es la version HA actual en `rainmapper-app/config.yaml`, `rainmapper-app/Dockerfile`, cache-busters y changelog, y esta publicada en GHCR como `0.2.132/latest` con digest multi-arch `sha256:801e77ff582afe64d47dd7f56935e424732ee687ab98b5436afaf789c49762ad`. Incluye la persistencia backend de settings Heatmap de `0.2.127`, restaura el comportamiento correcto del historial de popups MapLibre de `0.2.129`, conserva la prioridad de arranque de fuentes de `0.2.130` y corrige el problema de cabeceras sticky: `0.2.130` evito que se vieran filas por encima pero introdujo recorte de titulos, `0.2.131` no lo corrigio por completo y `0.2.132` elimina el solapamiento horizontal del fondo sticky entre columnas. Conserva la semantica de valores ausentes de `0.2.126`: `null`/vacío/no numerico es sin dato, no cero; los puntos sin dato se pintan gris con radio minimo y quedan fuera del heatmap; los ceros numericos reales siguen contando como cero. Tomap preserva `Total` ausente en grupos sin lluvia numerica, evitando fabricar `0` por `groupby().sum()`. Auditoria GitHub del 2026-06-24: el repo remoto estaba publico (`private=false`, `visibility=public`) para que HA detectase updates; esa visibilidad no se ha re-auditado en este cierre. Siguiente paso recomendado: instalar/actualizar `0.2.132` en HA, validar que el historial largo del popup no muestra filas por encima y que los titulos `Data/Dies/mm/°C/%/km/h` se ven completos; despues volver a poner el repo privado y limpiar GHCR remoto conservando solo la version validada/latest y sus auxiliares multi-arch.
