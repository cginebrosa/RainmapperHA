...
Seguir iterando la UI de especies de `/mushrooms/profiles` hacia `docs/mushrooms/ui/profiles/mushroom-profiles-species.png`, con cambios acotados en `rainmapper-app/app/mushroom_profiles_ui.py` y solo tocando `web_server.py` si cambian rutas/POST/orquestacion. No cambiar version HA ni publicar imagen nueva salvo peticion explicita. Mantener pendiente `docs/mushrooms/ui/reference-catalogs/reference-catalog-domain-impact-reference.png` como pantalla informativa futura.
