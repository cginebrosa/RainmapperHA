...
`0.2.136` es la version HA actual en `rainmapper-app/config.yaml`, `rainmapper-app/Dockerfile`, cache-busters y changelog. Esta publicada en GHCR como `0.2.136/latest` con digest multi-arch `sha256:1eb800cf155af3c194571f34118a86705494167477318681eca119d203c49148`, commit de release `d98887c` y queda pendiente de validar en HA. Corrige la incidencia de `0.2.135` donde los defaults HA del heatmap (`soft`, `65`, `90`, `70`) no se aplicaban a usuarios/dispositivos nuevos porque el backend sintetizaba settings de device ausentes con valores fijos (`65`, `100`, `100`); ahora los campos `heatmap_opacity`, `heatmap_radius_scale` y `heatmap_intensity_scale` no se devuelven si faltan en `devices.json`, para que el visor use `heatmapDefaults` de `config.yaml`. Tambien permite `maplibre_hover_zoom` decimal, por ejemplo `6.5`. Estado operativo: repo GitHub abierto temporalmente (`private=false`, `visibility=public`, rama `inicial`) para que Home Assistant detecte el update. Siguiente paso recomendado: instalar/validar `0.2.136` en HA; revisar usuario/dispositivo nuevo, boton `Reset heatmap defaults`, y `maplibre_hover_zoom` decimal. Si queda validada, volver a poner el repo privado y limpiar GHCR remoto conservando solo `0.2.136/latest` y auxiliares multi-arch. `0.2.134` queda como ultimo rollback validado previo; `0.2.135` no debe cerrarse como version buena por el bug de defaults/reset heatmap.
