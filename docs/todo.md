...
`0.2.123` es la version HA actual en `rainmapper-app/config.yaml`, `rainmapper-app/Dockerfile`, cache-busters y changelog, y esta publicada en GHCR como `0.2.123/latest` con digest multi-arch `sha256:89118fb88892ef2910eea09b8e72c3c460040d0d24d39b73801cd82d2a81a590`. Incluye el experimento MapLibre heatmap en una ruta publica separada (`/local/rainmapper-maplibre-heatmap/index.html`) sin cambiar el visor protegido por defecto: boton `Heatmap`, selector y boton rapido de metrica (`Rain`, `Max temp`, `Min temp`, `Max humidity`, `Min humidity`, `Wind`), pestaña `Heatmap` en Settings con metrica, opacidad, radio, intensidad y curva de peso (`Lineal`, `Suave`, `Fuerte`), puntos/leyenda por metrica, heatmap por encima de puntos y fuente heatmap independiente del filtro de lluvia minima pero filtrada por fuentes activas. Tambien conserva el fix `0.2.122` de lectura de incrementales grandes con inferencia completa para evitar los `DtypeWarning` de pandas vistos en `rainmapper.py` y `tomap.py`. `0.2.122` queda como version HA publicada con digest multi-arch `sha256:aeb39767d061671d1b75d810115d93e13dc9cd0776035e77f184ed3a577ee18e`. `0.2.121` queda como version HA publicada con digest multi-arch `sha256:dcc820f91e3f05d0d80dbe43f55213c821ea75292a3833f37fb38934ca5bb0aa`. `0.2.120` queda como version HA publicada con digest multi-arch `sha256:9efce7f58351b4c1f3634cc7ce33e9ef389310544dc887caa199fd310f0ab2ad`. Auditoria GitHub del 2026-06-24: el repo remoto esta publico (`private=false`, `visibility=public`) para que HA detecte updates. Siguiente paso recomendado: instalar/actualizar `0.2.123` en HA, validar visualmente el experimento heatmap y confirmar que desaparece el warning de pandas; despues volver a poner el repo privado y limpiar GHCR remoto conservando solo la version validada/latest y sus auxiliares multi-arch.
