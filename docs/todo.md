...
Nota de continuidad 2026-06-28: version HA actual `0.2.165` publicada y pendiente de validar en HA. Imagen `ghcr.io/cginebrosa/rainmapperha:0.2.165`, digest multi-arch `sha256:23faa9f9e57dfc82fccc177bd10fef308c965268d6eaf545fac55a16abc262cf`. Mejora la capa funcional y visual de mantenimiento de especies: `0.2.161` anade validacion semantica compartida para bloquear duplicados en arrays simples, IDs duplicados en afinidades y solapes entre `phenology.main_months` y `phenology.secondary_months`; `0.2.162` hace visibles esos bloqueos como alertas de `Validation error`; `0.2.163` anade alta guiada `New species` con plantilla completa en estado `draft`, `not_calibrated` y `requires_human_validation=true`; `0.2.164` inicia el redisenyo visual hacia `docs/mushrooms/ui/profiles/mushroom-profiles-species.png` y extrae helpers de render de especies a `rainmapper-app/app/mushroom_profiles_ui.py`; `0.2.165` amplia el ancho util global de la WebUI HA, pliega la validacion cruzada de especies, anade tabs superiores de seccion, corrige inputs decimales de scoring con `step=0.01`, limites `0..1` y soporte de coma decimal en formularios, y extrae el render del catalogo maestro a `rainmapper-app/app/mushroom_catalogs_ui.py`. `web_server.py` conserva rutas, POST, persistencia y orquestacion de validacion, pero ya no debe volver a absorber pantallas grandes de mantenimiento. El modulo comun de validacion es `rainmapper_core/mushroom_validation.py` y lo usan tanto la WebUI HA como `scripts/validate-mushroom-data.py`. `docs/mushrooms/ui/reference-catalogs/reference-catalog-domain-impact-reference.png` sigue pendiente como pantalla informativa/analitica futura, no bloquea el flujo principal de mantenimiento.
...
Validar `0.2.165` en HA. Foco visual: comprobar que `/mushrooms/profiles` aprovecha mejor el ancho de pantalla y se acerca al mockup `mushroom-profiles-species.png`: listado izquierdo compacto, cabecera de especie con chips, tab `General` tipo dashboard con tarjetas, meses como chips, barras de scoring y barra de acciones inferior. Foco funcional: comprobar que los pesos de scoring suben/bajan de `0,01` en `0,01`, que se acepta coma decimal, que la suma de pesos se muestra, que `Cross validation` queda plegado y que `New species` sigue creando una especie de prueba como `draft`/`not_calibrated`. Comprobar también que el validador sigue mostrando `0 errors · 7 warnings` con los datos actuales tras borrar o revertir la especie de prueba si se creó sólo para validación. Revisar que `/mushrooms/catalogs` conserva filtros, detalle friendly, cross references e import/export tras mover su render a `mushroom_catalogs_ui.py`.
