...
Nota de continuidad 2026-06-28: version HA actual `0.2.168` publicada y pendiente de validar en HA. Imagen `ghcr.io/cginebrosa/rainmapperha:0.2.168`, digest multi-arch `sha256:858e142e109ac1ca4c26068568fe44ff484e24e062bef88058d01c4bf91f171a`. `0.2.167` quedo instalada y probada por el usuario; debe considerarse avance visual funcional, pero queda pendiente revisar el funcionamiento completo de `Create species`, `Duplicate species` y `Archive species`: los dos ultimos aun no estan implementados y `Create species` queda pendiente de prueba explicita del usuario. Imagen `0.2.167`: `ghcr.io/cginebrosa/rainmapperha:0.2.167`, digest multi-arch `sha256:b8fff5a9013159295d62d38dfc5f39d9b407d99c1ef4262cc78b19a861a52887`. `0.2.168` mueve el panel guiado `New species` cerca de la parte superior para facilitar su prueba y marca `Duplicate species`, `Validate profile` y `Archive species` como acciones planificadas deshabilitadas para que no parezcan botones rotos. La UI de especies sigue en iteracion hacia `docs/mushrooms/ui/profiles/mushroom-profiles-species.png`. `web_server.py` conserva rutas, POST, persistencia y orquestacion de validacion; las pantallas grandes de setas viven en `rainmapper-app/app/mushroom_profiles_ui.py` y `rainmapper-app/app/mushroom_catalogs_ui.py`. El modulo comun de validacion es `rainmapper_core/mushroom_validation.py` y lo usan tanto la WebUI HA como `scripts/validate-mushroom-data.py`. `docs/mushrooms/ui/reference-catalogs/reference-catalog-domain-impact-reference.png` sigue pendiente como pantalla informativa/analitica futura, no bloquea el flujo principal de mantenimiento.
...
Validar `0.2.168` en HA. Foco principal: abrir `/mushrooms/profiles`, confirmar que `New species` aparece cerca de la parte superior y probar la creacion guiada de una especie draft. Confirmar que `Duplicate species`, `Validate profile` y `Archive species` aparecen como acciones planificadas/deshabilitadas, no como botones aparentemente rotos. Revalidar guardado normal de una especie desde `Metadata`, scoring decimal/validacion, raw JSON, import/export y navegacion hacia `/mushrooms/catalogs`.

- [ ] Revisar flujo `Create species`, `Duplicate species` y `Archive species`
  - Contexto: tras probar `0.2.167`, el usuario indica que `Duplicate species` y `Archive species` no funcionan, y quiere probar expresamente `Create species`. En `0.2.168` se aclara visualmente que duplicar/archivar/validar perfil son acciones planificadas y no implementadas, y se sube el panel `New species` para facilitar la prueba.
  - Criterio de aceptacion futuro: `Create species` debe crear una especie draft validada y abrirla; `Duplicate species` debe pedir nuevo ID/nombre, clonar desde la especie actual, reiniciar metadata/calibracion segun reglas de mantenimiento y validar antes de persistir; `Archive species` debe usar el flujo seguro de archivado/restauracion con backup y confirmacion explicita.
  - Estado: pendiente de revision funcional y diseno de salvaguardas antes de implementarlo completamente.

- [ ] Implementar archivado/restauracion de especies
