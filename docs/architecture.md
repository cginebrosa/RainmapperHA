...
- El paquete remoto GHCR debe seguir accesible para Home Assistant si no se configura autenticacion de registry en HA. Estado verificado por el flujo de release local de esta sesion: `0.2.132/latest` se publico con digest multi-arch `sha256:801e77ff582afe64d47dd7f56935e424732ee687ab98b5436afaf789c49762ad`. La visibilidad y limpieza remota de GHCR no se han re-auditado en este cierre; el script `scripts/build-push-ha-image.sh` solo limpia etiquetas locales y no borra versiones remotas.
- Procedimiento estandar tras publicar y validar una nueva version HA: limpiar tambien las versiones remotas antiguas del paquete GHCR, conservando solo la ultima version validada, `latest` y las entradas auxiliares sin tag asociadas al mismo push multi-arch. Esto evita acumular basura en GitHub Packages. No hacer esta limpieza antes de confirmar que HA descarga, arranca y queda validada visualmente con la nueva version; en este cierre no limpiar remoto hasta validar `0.2.132`.
