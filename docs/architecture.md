...
- Usuarios manuales en `/share/rainmapper/users.json`, con `username`, `name`, `email`, `password`, `role`, `enabled`, `max_devices`, `must_change_password`, `can_use_heatmap` y `can_use_layer_metrics`. `username` es el identificador de login; `name` es el nombre de la persona; `email` es contacto.
...
- `can_use_heatmap` autoriza el boton `Heatmap` y la pestana/seccion `Heatmap` en Settings; `can_use_layer_metrics` autoriza el boton rapido de metrica/capa y el selector `Layer metric`. Si faltan en usuarios existentes, se aplican defaults compatibles: usuarios `admin` con ambos permisos activos y resto de roles sin ellos. Al crear usuarios `admin`, la WebUI activa ambos permisos por defecto.
- Limitacion consciente: estos permisos estan embebidos ahora en cada usuario porque solo hay pocas funcionalidades protegidas. Si crece el numero de capacidades con permisos, revisar la arquitectura hacia perfiles/tipos de usuario con permisos declarados en un JSON separado, manteniendo `users.json` para identidad y overrides opcionales por usuario.
...
- El repo se anadio como repositorio de apps/add-ons en HA cuando era publico. Desde el 2026-06-22 se decidio mantener el repo GitHub `cginebrosa/RainmapperHA` privado para no exponer codigo ni logica de descarga, abriendolo solo en ventanas operativas para que HA detecte updates. Tras validar `0.2.132` en HA, auditoria final del 2026-06-25: repo privado (`private=true`, `visibility=private`, rama `inicial`).
...
- El paquete remoto GHCR debe seguir accesible para Home Assistant si no se configura autenticacion de registry en HA. `0.2.133/latest` se publico y valido en HA el 2026-06-25 con digest multi-arch `sha256:d0b6432a123b718a5ad58ed990b48a7847589b8e74d4487ea1010787d2a775f2`. Tras la limpieza remota, GHCR conserva solo `0.2.133,latest` y cuatro auxiliares sin tag del mismo push multi-arch.
