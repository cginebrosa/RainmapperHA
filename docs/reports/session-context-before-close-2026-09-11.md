# Archivo de contexto anterior al cierre del 11/09/2026

Snapshot histórico; NO es contexto operativo ni autorización vigente. Contiene
pendientes y estados posteriormente reemplazados. Para continuar, leer
`docs/codex-start-here.md` y `docs/active-context.md`. Las comprobaciones antiguas
solo acreditan el estado en el momento en que se realizaron.

## Contexto previo (copia íntegra)
...
## Investigación: Mapa de predicción

- Nombre acordado con el usuario: **Mapa de predicción** para el desarrollo nuevo
  por coordenadas; **Predictor** queda reservado para la herramienta actual.
- [Seguimiento por pasos](mushrooms/mushroom-prediction-map-progress-es.md): paso 1
  de geología ICGC preparado en `mushroom-map-GIS/icgc-geologia-50000/source/`.
  Copias de los originales verificadas, 61.437 polígonos con índice, 16 controles
  locales catalanes positivos y tres exteriores negativos. Informe en
  `docs/reports/mushroom-prediction-map-geology-icgc-2026-09-11.json`.
  El usuario requiere geología **local**, sin dependencia de servidores. GEODE
  también está descargado, pendiente de preparar índice local; consultas online
  realizadas solo para diagnóstico. No hay integración ni cambios al Predictor.
  Siguiente paso: diseñar una base SoilGrids local compartida para Predictor y
  Mapa de predicción, como plantea el usuario, ampliando territorio y propiedades
  sin duplicar los archivos existentes. Hay 30 teselas distintas de retención;
  el lector actual exige sus 54 capas. [Dimensionado concretado](mushrooms/mushroom-prediction-map-soilgrids-plan-es.md):
  retención + solo pH superficial con incertidumbre, 63 combinaciones y 113
  teselas conservadoras; 3,73 GB de píxeles sin comprimir. Textura/carbono/materia
  orgánica fuera del alcance inicial. El usuario pide una precisión útil,
  compatible con meteorología IDW: detalle del terreno no implica pronóstico
  exacto a escala de metros; tamaño de zona meteorológica pendiente de auditar.
  Adquisición nacional terminada y verificada en
  `mushroom-map-GIS/soilgrids-shared/source/`: 538 bloques nuevos y 1.410 archivos
  previos copiados con verificación de huellas; 795.522.040 bytes de rásteres.
  Los 7.119 pares tesela/capa tienen archivo y referencia válidos. Todos los
  bloques nuevos pasan lectura GDAL y SHA-256; no quedan `.part`.
  [Evidencia](reports/mushroom-prediction-map-soilgrids-acquisition-2026-09-11.json).
  Estado persistido en
  `soilgrids-shared/acquisition-status.json`; reanudación con
  `.venv/bin/python mushroom-map-GIS/acquire-soilgrids.py` (bloqueo contra duplicados).
  [Cobertura auditada](mushrooms/mushroom-prediction-map-soilgrids-coverage-es.md):
  97,5675 % español y 95,4419 % catalán con valores utilizables en las 63 capas;
  los 66 contextos de microárea conservan geometría y referencias, y los 1.355
  rásteres normalizados coinciden en ambas raíces. Usuario acepta huecos:
  no rellenarlos. Ceros WCS contrastados con NoData original de pH en controles;
  no extrapolar esa equivalencia a cualquier cero aislado de retención.
  Prioridad del usuario: eficiencia RPi4. Archivo máximo 4,14 MB, todos los
  rásteres por bloques 256×256. La nueva consulta debe resolver por índice y
  leer ventanas sin repetir hashes completos/procesos por capa y clic; no
  ejecutar la auditoría nacional en HA. Rendimiento real RPi4 aún no medido.
  Pendientes: normalización y lector compatible eficiente, validación local y migración
  validada; primero descarga, luego cambio controlado, después backup temporal
  del antiguo antes de revisar su borrado. Propuesta de retención: mínimo 30 días.
  No se cambia el lector ni se eliminan datos operativos durante la descarga.

- El usuario solicita un desarrollo complementario al Predictor y autoriza
  investigar/documentar su viabilidad, incluyendo consultas de Sporas en Safari.
  La navegación de consulta está autorizada sin confirmación por cada paso;
  no modificar setales ni configuración de la cuenta.
- [Análisis y propuesta](mushrooms/mushroom-map-point-prediction-feasibility-es.md):
  reutilización GIS/IDW/modelos, cobertura real, literatura para 21 perfiles,
  temporada configurada frente a aprendida y validación de una candidata
  geográfica separada. Recomendación inicial: compatibilidad explicada y prueba
  a siete días; 15 días con previsiones requiere otro contrato y evaluación.
- [Inventario local](reports/mushroom-map-point-inventory-2026-09-11.json):
  470 observaciones, 427 con filtro inicial de inclusión, 34 áreas y 66 microáreas.
  Tres consultas GIS de lectura confirman diferencias de cobertura; no son una
  auditoría completa del territorio. La caché de retención no incluye pH.
- Alcance concretado: Catalunya primero y España como ampliación deseada.
  Propuesta afinada: evidencia global de especie como selección principal en
  todos los puntos; validar aparte el cambio de escala de las entradas. Ocho
  controles catalanes devuelven DEM, vegetación y geología; metadatos y resultados
  en [cobertura territorial](reports/mushroom-map-point-coverage-2026-09-11.json).
  Fuentes españolas candidatas: IGN/CNIG, MFE25 y GEODE. Adquisición autorizada
  por el usuario, separada de `mushroom-GIS` en **`mushroom-map-GIS/`**.
- Descargas del 11/09: ICGC cubiertas 2024 completo (1.524.399 polígonos), IGN
  MDT25 primera cobertura completo (1.524 TIFF; 4.591.983.594 bytes; cabeceras
  GDAL verificadas). Dos discrepancias nombre/CRS documentadas. MFE25: tablas
  complementarias y los 17 paquetes regionales recibidos y comprobados, aportados
  por el usuario desde el navegador (15 carpetas, ZIP Catalunya y RAR Galicia).
  Total: 1.994.853 polígonos, 153 originales, 18.004.586.141 bytes extraídos;
  apertura GDAL, atributos por territorio/origen y SHA-256 registrados. No quedan
  comunidades pendientes del catálogo. `MFE_42` renombrada a `mfe_castillalamancha`
  por petición del usuario. Diccionarios 2021/2022/2023/2025 y campos variables:
  normalizar antes de integrar. Canarias WGS84/UTM28N; las otras 16 ETRS89/UTM30N.
  No se ha auditado exhaustivamente la geometría ni ausencia de huecos.
  GEODE completo: 612.170 recintos geológicos y 44.464 de Cuaternario,
  658 bloques, 1.625.044.963 bytes comprimidos; conteos e identificadores
  verificados. [Inventario de adquisición](reports/mushroom-map-gis-downloads-2026-09-11.json).
- Calidad GIS nueva: GEODE no contiene registros Z3000/Catalánides en su capa
  principal pese a incluir esa zona en el catálogo; no asumir cobertura completa
  catalana. Un registro de Cuaternario (OBJECTID 22583) tiene geometría vacía.
  Se conservan originales y se documentan límites antes de una integración.
- README junto a cada fuente/descarga, como pide el usuario; los TIFF IGN tienen
  también README individual. Datos excluidos de Git y del contexto Docker.
  No están integrados en el Predictor. No se implementa el módulo ni se inician
  entrenamientos, precálculos o releases. Observaciones y destinos se conservan.
