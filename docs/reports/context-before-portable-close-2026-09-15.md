## Pausa solicitada — preparación portable, 15/09/2026

El usuario pide detenerse **antes de mover archivos en HA real**, porque viaja
por carretera a Barcelona. No iniciar operaciones en `/Volumes/media` hasta que
indique que podemos retomar. En esta fase no se ha movido ni reorganizado ningún
archivo remoto; las copias verificadas anteriores y los tres originales siguen
como estaban.

Corrección de diseño vigente: preparar archivos ordinarios y manifiestos en sus
rutas definitivas **antes** de instalar HA. La aplicación los lee directamente;
no habrá comandos, adopción, enlaces ni migraciones en el primer arranque.
Las instrucciones anteriores de adopción nativa quedan pendientes de sustituir.

Trabajo local en curso (no aceptado todavía): lectura portable de identidades y
referencias a archivos compartidos; preferencia por
`/media/rainmapper/geography/mushroom-GIS`; configuración del mapa preparada en
`geography/map-config.json`. Fixture local independiente en
`docker-media/rainmapper/geography-portable` (3.581 archivos, 15.622.243.791 bytes
lógicos, copias APFS, sin hardlinks). Plan remoto **sin ejecutar** en
`tmp/geography-portable-20260915/remote-plan.json`: reorganizar únicamente nuestras
copias de `geography/imports`, conservando los tres directorios originales.

Pruebas dirigidas: 25 pasan (incluyen traslado de paquete sin escrituras al abrir,
rechazo de cambios y reutilización de caché mapa/dataset). Imágenes locales HA y
worker reconstruidas; **contenedores aún sin recrear**, por tanto todavía ejecutan
el código anterior. La comparación de cuatro puntos se detuvo antes de leer al
usar una firma de función nueva con la imagen antigua; no cuenta como validación.
Smoke local iniciado, resultado pendiente de recoger en
`tmp/geography-portable-20260915/smoke.log`.

Retomar desde `tmp/geography-portable-20260915/RESUME.md`. No publicar ni instalar
HA real. Preservar URLs de coordinadores y trabajos del usuario.

# Active Context

Estado actualizado al **15/09/2026**, tras completar la aceptación local de la candidata HA 0.2.304 / worker 1.1.2. Leer completo `codex-start-here.md` y este documento;
`todo.md` solo amplía prioridades. Este contexto basta para retomar. Detalles
históricos preservados en [el snapshot anterior](reports/prediction-map-context-before-two-level-close-2026-09-14.md);
no hace falta leerlo para continuar.

## Consolidación GIS aceptada localmente — 15/09

Almacén común por SHA implementado en `mushroom_geography_store.py`; adopción
explícita en `mushroom_geography_import.py` / `manage-shared-geography.py`.
Mapa y dataset del worker comparten objetos físicos mediante enlaces. Recibos
SQLite y metadatos nativos separan identidad física del mtime lógico de los índices.
Heartbeat y snapshots no rehashean GiB. El dataset científico sigue enviando solo
13 archivos; SoilGrids y otros auxiliares se conservan en la vista GIS de HA.

**HA real:** dos entradas copiadas y verificadas desde Mac, sin activar ni instalar:
`geography/imports/local-20260914` (3.510 archivos / 14.538.214.301 bytes) y
`geography/imports/legacy-gis-20260915` (delta de 1.084.029.490 bytes; reutiliza
5.736.287.356 bytes de la primera). Los 1.432 archivos antiguos se comprobaron
contra sus hashes. Todos los originales remotos permanecen intactos.

**Local:** ambas vistas publicadas bajo `/media/rainmapper/geography`;
configuración del mapa, GIS y SoilGrids usan el almacén compartido.
Limpieza aplicada con servicios parados y sin trabajos: worker liberó
5.457.006.592 bytes asignados; HA local 4.009.984 bytes. Eliminadas únicamente
la caché worker `map-geography` y las entradas locales `prediction-map` y
`geography-import-verified`. Los archivos grandes de HA local ya eran enlaces.
Se conservaron versiones científicas, punteros activos, datos privados, backups,
coordinadores y fuentes del repositorio. No se tocó el volumen anónimo vacío.

Validación: smoke 1.594 tests / 48 skips; tras el último cambio del importador
incremental, seis tests dirigidos. Cuatro puntos GIS con igualdad total de valores
y contexto. Circuito local completo: reconstrucción y base promocionadas,
714/714 ajustes multiversión correctos y precálculo verificado/activado (27.602.944
bytes, revisión 63). Ambas imágenes finales reconstruidas y recreadas; código
efectivo idéntico al worktree en 197 archivos HA y 106 worker.
Tras limpiar/reiniciar, seis consultas autenticadas con paridad local/worker,
usuario básico, cancelación y concurrencia correctos. Lote usado:
`operational_20260915T155407Z`. Recibos válidos: 3.416 objetos HA / 3.352 worker.
Worker sincronizado con 0 bytes de payload, metadatos y hash tras reinicio.
URLs persistidas de ambos coordinadores sin cambios.

[Informe final](reports/shared-geography-acceptance-2026-09-15.json) y
[guía de estructura/adopción](mushrooms/shared-geography-consolidation-es.md).
No repetir copias ni circuito local ya terminado por cambios solo documentales.
Pendiente: aceptación del usuario para release según `release-flow.md`, adopción
nativa/publicación en HA real e iPhone. El usuario instalará la versión y borrará
allí lo sobrante tras verificar. No se ha publicado ni instalado remotamente.

## Copia del usuario a HA real — comprobación del 15/09

El usuario copió la cartografía descomprimida y autorizó su comprobación. Lectura
por SMB de `/Volumes/media` (HA real `100.111.77.48`): ruta correcta,
3510/3510 archivos, 14.538.214.301 bytes, sin faltantes ni tamaños incorrectos;
manifiesto idéntico al paquete sellado. Sin lecturas/hash de los rásteres ni
escrituras en HA real. [Informe](reports/prediction-map-ha-real-copy-2026-09-15.json).
SMB muestra diferencias de precisión en 3493 timestamps; comprobar los timestamps
nativos al publicar desde HA y usar `--restore-timestamps` si hace falta.
No se ha activado la cartografía ni instalado/publicado una versión nueva.
Esta comprobación documental posterior no cambia la candidata ni invalida sus tests.
