...
| Investigación WU | [servidor](../../scripts/station_research.py), [visor](../../scripts/station-research/app.js) | SQLite, cuatro estados, filtros combinables, deduplicación por ID, 30 días completos, promoción solo a candidata. Los recuentos iniciales no son estado actual de revisiones. |
| Revisión GBIF | [visor](../mushrooms/GBIF/gbif-viewer.js), [guía](../mushrooms/GBIF/README.md) | Cuatro estados; `approved` corresponde a Aceptada y no debe confundirse con `accepted` de WU. No importación operativa automática. |
