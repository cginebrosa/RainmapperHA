...
- El repo GitHub es privado; el paquete GHCR debe quedar accesible para HA. El estado remoto exacto de GHCR no se puede verificar desde el repo; tras validar una version HA, hay que limpiar versiones remotas antiguas conservando solo la version vigente, `latest` y las entradas auxiliares del mismo push multi-arch.
