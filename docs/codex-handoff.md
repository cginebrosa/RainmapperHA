...
- Estado actual: version `0.2.147` publicada en GHCR y pusheada a GitHub, pendiente de validar en HA. La app mantiene ingress, sidebar, imagen preconstruida `ghcr.io/cginebrosa/rainmapperha`, opciones de schedule, Google Maps API key, AEMET API key, mapas, fuentes, publish, `maplibre_hover_zoom`, defaults HA de heatmap y defaults/tunables HA de IDW (`maplibre_estimated_field_*`). La webUI muestra la version runtime en el panel de estado, un Control Panel compacto con tabs internos, estado separado por fuente (`Meteoclimatic`, `Meteocat`, `Wunderground`, `AEMET`) y enlaces de visores con cache-buster de version para evitar cargas obsoletas en HA. La validacion de `Run all`, logs en ingles y schedule en la instalacion real de Home Assistant es manual/reportada por el usuario; pendiente de confirmar automaticamente para `0.2.147`. `0.2.146` queda validada/dada por buena para la WebUI Users redisenada y `0.2.137` sigue siendo el rollback cerrado previo al bloque IDW. `0.2.134` queda como rollback visual validado previo.
...
- Estado actual: usa Python 3.11 slim. Version alineada con `rainmapper-app/config.yaml` en `0.2.147`; tambien copia `users.example.json` para inicializar usuarios si falta configuracion persistente.
...
- Versionado HA: `config.yaml`, labels Docker, banner runtime y cache-busters de visores estan alineados en `0.2.147`.
...
- Imagen Docker HA multi-arch preconstruida en GHCR desde `0.2.57`; el repo confirma `image: ghcr.io/cginebrosa/rainmapperha` y el script `scripts/build-push-ha-image.sh`. La instalacion rapida en HA, el progreso de Supervisor, la poca utilidad del cache de GitHub Actions y la limpieza local observada son validaciones manuales/reportadas por el usuario; pendientes de confirmar automaticamente. Desde `0.2.60`, el flujo normal documentado es publicar la imagen desde el Mac con `scripts/build-push-ha-image.sh` antes de subir el commit de version; GitHub Actions queda como fallback manual. `0.2.147` y `latest` se publicaron en GHCR el 2026-06-27 con digest multi-arch `sha256:368c910b9a31fba587c1e1cbca0201395feeecca3bf9e8884f62ccc08a76feef`, pendiente de validar en HA. `0.2.146` queda validada/dada por buena para la WebUI Users redisenada. `0.2.137` sigue siendo rollback cerrado previo al bloque IDW, con digest `sha256:539c879d2c7f9dfc282d671b71c627a858b48d59778e3195ec2d0254accee928`. `0.2.134` queda como rollback visual validado previo. `0.2.132` queda como rollback operativo anterior validado. `0.2.129` se conserva como rollback visual previo documentado; `0.2.130` y `0.2.131` no son buenos rollback visuales para el popup.
