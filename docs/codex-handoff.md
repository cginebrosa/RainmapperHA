...
- La imagen HA publicada documentada para `0.2.149/latest` es `ghcr.io/cginebrosa/rainmapperha` con digest multi-arch `sha256:3a488f597e34d2caba2c30edc90f5426813eb0c19858e2dcd679b197abda474b`; commit de release `039e615`; instalada y validada/dada por buena en HA para el fix de User Management.
...
- Estado actual: version `0.2.149` publicada en GHCR, pusheada a GitHub, instalada y validada/dada por buena en HA para User Management. La app mantiene ingress, sidebar, imagen preconstruida `ghcr.io/cginebrosa/rainmapperha`, opciones de schedule, Google Maps API key, AEMET API key, mapas, fuentes, publish, `maplibre_hover_zoom`, defaults HA de heatmap y defaults/tunables HA de IDW (`maplibre_estimated_field_*`). La webUI muestra la version runtime en el panel de estado, un Control Panel compacto con tabs internos, estado separado por fuente (`Meteoclimatic`, `Meteocat`, `Wunderground`, `AEMET`), enlaces de visores con cache-buster de version y User Management compacto tipo accordion con panel expandido reorganizado. La validacion de `Run all`, logs en ingles y schedule en la instalacion real de Home Assistant es manual/reportada por el usuario; pendiente de confirmar automaticamente para `0.2.149`. `0.2.148` no debe cerrarse como buena por el bug visual del acordeon totalmente desplegado; `0.2.147` fue reportada por el usuario como funcionando bien para el Control Panel redisenado, `0.2.146` queda validada/dada por buena para la WebUI Users redisenada y `0.2.137` sigue siendo el rollback cerrado previo al bloque IDW. `0.2.134` queda como rollback visual validado previo.
...
- Estado actual: usa Python 3.11 slim. Version alineada con `rainmapper-app/config.yaml` en `0.2.149`; tambien copia `users.example.json` para inicializar usuarios si falta configuracion persistente.
...
- Versionado HA: `config.yaml`, labels Docker, banner runtime y cache-busters de visores estan alineados en `0.2.149`.
