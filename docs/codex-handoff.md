...
- Estado actual: usa Python 3.11 slim. Version alineada con `rainmapper-app/config.yaml` en `0.2.112`; tambien copia `users.example.json` para inicializar usuarios si falta configuracion persistente.
...
- Versionado HA: `config.yaml`, labels Docker, banner runtime y cache-busters de visores estan alineados en `0.2.112`.
...
- El repo GitHub es privado; el paquete GHCR debe quedar accesible para HA. Estado remoto verificado el 2026-06-24: `0.2.112` esta publicada y verificada como multi-arch, pero aun pendiente de validacion HA; conservar `0.2.111` como fallback validado hasta que `0.2.112` arranque correctamente. Tras futuras versiones HA validadas, limpiar versiones remotas antiguas conservando solo la version vigente, `latest` y las entradas auxiliares del mismo push multi-arch.
