...
- Estado actual: usa Python 3.11 slim. Version alineada con `rainmapper-app/config.yaml` en `0.2.118`; tambien copia `users.example.json` para inicializar usuarios si falta configuracion persistente.
...
- Versionado HA: `config.yaml`, labels Docker, banner runtime y cache-busters de visores estan alineados en `0.2.118`.
...
- El repo GitHub deberia quedar privado salvo ventanas operativas para que HA detecte updates; auditoria real del 2026-06-24 indica que ahora mismo esta publico (`private=false`, `visibility=public`). El paquete GHCR debe quedar accesible para HA. Estado remoto verificado el 2026-06-24: GHCR conserva `0.2.118/latest` y `0.2.117` como rollback, cada una con sus cuatro entradas auxiliares del push multi-arch. Tras validar `0.2.118`, volver a poner el repo privado y limpiar versiones remotas antiguas conservando solo la version vigente, `latest` y las entradas auxiliares del mismo push multi-arch.
