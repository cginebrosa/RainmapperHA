...
- Estado actual: usa Python 3.11 slim. Version alineada con `rainmapper-app/config.yaml` en `0.2.122`; tambien copia `users.example.json` para inicializar usuarios si falta configuracion persistente.
...
- Experimento publicado en `0.2.121`: existe una variante publica separada `/local/rainmapper-maplibre-heatmap/index.html`, activada por `window.RAINMAPPER_CONFIG.experimentalHeatmap`, para probar heatmap sin afectar a los usuarios del visor protegido. Mantiene los puntos filtrados por lluvia minima/fuente, pero el heatmap usa una fuente GeoJSON independiente con todas las estaciones validas del periodo filtradas por fuente activa. El experimento incluye boton `Heatmap`, selector `Layer metric` (`Rain`, `Max temp`, `Min temp`, `Max humidity`, `Min humidity`, `Wind`), boton rapido de metrica similar al panel rapido de capas, sliders de opacidad/radio y leyenda reutilizada para la metrica activa. La metrica seleccionada afecta siempre a puntos y leyenda; el boton `Heatmap` solo anade o quita la capa de densidad. En `0.2.121`, el heatmap se amplia con radio ajustable y se dibuja por encima de los puntos para inspeccionar mejor la densidad. No persistir aun esas preferencias porque la ruta es experimental; si se promociona a produccion, `heatmap` activo/inactivo, metrica, opacidad y radio deben persistirse por `device_id` en `devices.json` con saneado backend y defaults compatibles.
