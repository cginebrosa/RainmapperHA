...
Nota de continuidad 2026-06-27: `0.2.149` corrige el acordeon de User Management de `0.2.148`. Causa: `.user-panel { display: grid; }` pisaba el atributo HTML `hidden`, y el JS restauraba/abría automaticamente un usuario al cargar. Fix: `.user-panel[hidden] { display: none; }`, retirada de la restauracion por `sessionStorage` y carga/refresco con todos los usuarios cerrados. `setExpandedUser()` mantiene la regla de una sola lamina abierta a la vez; pulsar la abierta la cierra. Validacion local antes de publicar: `python3 -m unittest tests.test_web_server_auth` OK y `./scripts/smoke-test.sh` OK. Pendiente de validacion visual/operativa en HA.
...
Actualizacion HA `0.2.149` publicada el 2026-06-27: imagen `ghcr.io/cginebrosa/rainmapperha:0.2.149` y `latest` con digest multi-arch `sha256:3a488f597e34d2caba2c30edc90f5426813eb0c19858e2dcd679b197abda474b`; commit de release `039e615`. Cambios principales: fix del acordeon de User Management para que todos los usuarios nazcan cerrados, `hidden` no quede pisado por `.user-panel { display: grid; }`, no se restaure automaticamente un usuario abierto por `sessionStorage`, y abrir un usuario cierre los demas. Validacion local antes de publicar: `python3 -m unittest tests.test_web_server_auth` OK y `./scripts/smoke-test.sh` OK. Pendiente de validacion visual/operativa en HA.
