...
- Estado actual: usa Python 3.11 slim. Version alineada con `rainmapper-app/config.yaml` en `0.2.148`; tambien copia `users.example.json` para inicializar usuarios si falta configuracion persistente.
...
- Versionado HA: `config.yaml`, labels Docker, banner runtime y cache-busters de visores estan alineados en `0.2.148`.
...
- El repo GitHub queda privado salvo ventanas operativas para que HA detecte updates. Estado actual: `0.2.148` esta publicada/pusheada y pendiente de validar en HA; `0.2.147` fue reportada como funcionando bien y `0.2.146` queda validada/dada por buena. Tras validar `0.2.137`, el repo quedo privado (`private=true`, `visibility=private`, rama `inicial`) y GHCR limpio conservando solo `0.2.137/latest` con digest multi-arch `sha256:539c879d2c7f9dfc282d671b71c627a858b48d59778e3195ec2d0254accee928` y cuatro auxiliares sin tag del mismo push multi-arch. Tras validar una version posterior, repetir limpieza remota.
