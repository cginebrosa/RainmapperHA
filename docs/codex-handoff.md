...
- Cambio local no publicado tras `0.2.114`: los periodos agregan max/min de temperatura/humedad, viento medio ponderado por `wind_observation_count` si existe, racha maxima y direccion media circular; `LastXX_rains.csv` anade `Hum_*_NN` y `Wind_*_NN` para detalle diario, manteniendo compatibilidad con CSV/dataframes antiguos sin columnas `wind_*`.
...
- Estado actual: usa Python 3.11 slim. Version alineada con `rainmapper-app/config.yaml` en `0.2.113`; tambien copia `users.example.json` para inicializar usuarios si falta configuracion persistente.
...
- Cambio local no publicado tras `0.2.114`: los popups muestran resumen de periodo con temperatura, humedad, viento medio/direccion y racha si existen; el historial diario muestra lluvia, temperatura, humedad y viento en una tabla compacta. La ayuda `?` y traducciones ES/EN/CA ya se actualizaron.
...
- Versionado HA: `config.yaml`, labels Docker, banner runtime y cache-busters de visores estan alineados en `0.2.113`.
...
- Imagen Docker HA multi-arch preconstruida en GHCR desde `0.2.57`; el repo confirma `image: ghcr.io/cginebrosa/rainmapperha` y el script `scripts/build-push-ha-image.sh`. La instalacion rapida en HA, el progreso de Supervisor, la poca utilidad del cache de GitHub Actions y la limpieza local observada son validaciones manuales/reportadas por el usuario; pendientes de confirmar automaticamente. Desde `0.2.60`, el flujo normal documentado es publicar la imagen desde el Mac con `scripts/build-push-ha-image.sh` antes de subir el commit de version; GitHub Actions queda como fallback manual. El 2026-06-24 se publico y valido/dio por buena `ghcr.io/cginebrosa/rainmapperha:0.2.113` y `latest` con digest multi-arch `sha256:b8bdf0a9b433932c4fc7af012cd7d0876ea6d821aa7131b5e81458031c831627`, verificado como index OCI con `linux/amd64` y `linux/arm64`. Tras validarlo en HA, se limpio GHCR y quedaron solo `0.2.113`, `latest` y cuatro entradas auxiliares del mismo push. El 2026-06-24 tambien se publico `0.2.112` con digest multi-arch `sha256:37f841c9004ab879227d2cc67ee6f836d1e8c4adc14ae609ba9b7cf41b3637f7`, superado antes de validarse. El 2026-06-23 se publico y valido/dio por buena `ghcr.io/cginebrosa/rainmapperha:0.2.111` y `latest` con digest multi-arch `sha256:f962cb744e41200badbe65786b8182f6c2ac6fe567e4e9e3415708ce4a250112`.
...
- El repo GitHub es privado salvo ventanas operativas para que HA detecte updates; el paquete GHCR debe quedar accesible para HA. Estado remoto verificado el 2026-06-24: GHCR conserva solo `0.2.117`, `latest` y las cuatro entradas auxiliares del mismo push multi-arch. Tras futuras versiones HA validadas, limpiar versiones remotas antiguas conservando solo la version vigente, `latest` y las entradas auxiliares del mismo push multi-arch.
...
Validado manualmente en HA hasta `0.2.113`: el visor MapLibre protegido restaura settings por dispositivo desde `devices.json`, guarda cambios solo al cerrar Settings tras modificar controles del panel, cambia mapa desde el boton rapido de layers sin persistir `map_style`, mantiene la separacion entre periodo visible y periodo preferido, permite guardar explicitamente `map_view` como vista predeterminada sin escribir continuamente al mover el mapa, soporta selector de idioma ES/EN/CA, muestra controles moviles compactos, la WebUI `Users` refresca/busca correctamente sin perder scroll, AEMET esta integrado en el visor protegido estandar, no duplica diarios por tipos mixtos en `local_date`, y `source_status.json` incluye `stations`. `0.2.113` queda validada/dada por buena en HA con ayuda movil MapLibre ajustada y badges de fuente en dos lineas. El 2026-06-24 se publico `0.2.114` en GHCR con digest multi-arch `sha256:24736a7b4f6b9a64a65a586cb41ed7b378efc2cbd8c9e0634b0152cdad49a9d1`; despues se valido que preserva el backfill manual AEMET tras un run AEMET. `0.2.115` no debe considerarse buena: en HA `Generate maps` subio a ~4:28 y el popup MapLibre quedo ilegible. `0.2.116` corrigio el rendimiento de Tomap y el usuario reporto `Generate maps` en 1:59, pero el detalle diario del popup aun necesitaba una fila compacta por dia. `0.2.117` queda publicada en GHCR con digest multi-arch `sha256:e12749d4b16a48c362f731eb4f03dbb850b71988061602396c51293ad0350d65`; mejora el historial diario MapLibre, pero perdio la columna de dias y la cabecera sticky al hacer scroll. `0.2.118` queda publicada en GHCR con digest multi-arch `sha256:07ce37c45de5f705aeb1621f4fb680a7b2c9360014ee1ccbb95322e7815d0e96`; restaura columna de dias y cabecera sticky manteniendo una fila por dia. Siguiente paso recomendado: instalar/actualizar `0.2.118` en HA y validar visualmente los popups en escritorio y movil; no limpiar `0.2.117` de GHCR hasta confirmar que `0.2.118` queda buena.

Actualizacion del 2026-06-24: tras subir el backfill AEMET de `tmp/aemet-backfill-0.2.114-output/Aemet_incremental.csv`, un run AEMET en HA preservo el rango `20260525`-`20260624`, no creo duplicados y relleno viento en filas recientes generadas desde horario. Despues se publico `0.2.115` en GHCR con propagacion de humedad/viento a Tomap/GeoJSON y su visualizacion en popups MapLibre, pero en HA se observo `Generate maps` demasiado lento (~4:28) y la tabla del popup se veia mal en desktop/movil; no considerar `0.2.115` como buena. `0.2.116` queda publicada en GHCR con agregacion Tomap de periodo vectorizada y popup MapLibre mas compacto; digest multi-arch `sha256:55d44ad51bb69c4d408ed6f4b9870b615e603665f6247245a5dd6a24740ecb62`. El usuario instalo/probo `0.2.116` y reporto `Generate maps` en 1:59, pero pidio mantener una fila por dia en el detalle diario. `0.2.117` queda publicada en GHCR con historial diario MapLibre en tabla compacta de cinco columnas (`Data`, `mm`, `°C`, `%`, `km/h`) y direccion de viento cardinal (`O/NO/SO` en ES/CA, `W/NW/SW` en EN); digest multi-arch `sha256:e12749d4b16a48c362f731eb4f03dbb850b71988061602396c51293ad0350d65`. Despues de publicar `0.2.117`, se limpio GHCR remoto: quedaron solo `0.2.117`, `latest` y cuatro entradas auxiliares del mismo push. El usuario valido que `0.2.117` se veia mucho mejor, pero faltaban la columna de dias y la cabecera sticky al hacer scroll. `0.2.118` queda publicada en GHCR restaurando ambas cosas; digest multi-arch `sha256:07ce37c45de5f705aeb1621f4fb680a7b2c9360014ee1ccbb95322e7815d0e96`. Validacion local: `scripts/smoke-test.sh`, `git diff --check` e inspeccion remota GHCR OK. Siguiente paso recomendado: actualizar HA a `0.2.118` y validar visualmente los popups MapLibre en movil/escritorio. No limpiar `0.2.117` de GHCR hasta que `0.2.118` este validada.
