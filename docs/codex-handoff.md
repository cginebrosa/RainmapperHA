...
- Estado actual: usa Python 3.11 slim. Version alineada con `rainmapper-app/config.yaml` en `0.2.119`; tambien copia `users.example.json` para inicializar usuarios si falta configuracion persistente.
...
- Experimento local tras `0.2.119`: existe una variante publica separada `/local/rainmapper-maplibre-heatmap/index.html`, activada por `window.RAINMAPPER_CONFIG.experimentalHeatmap`, para probar heatmap sin afectar a los usuarios del visor protegido. Mantiene los puntos filtrados por lluvia minima/fuente, pero el heatmap usa una fuente GeoJSON independiente con todas las estaciones validas del periodo. El experimento incluye boton `Heatmap`, selector `Layer metric` (`Rain`, `Max temp`, `Min temp`, `Max humidity`, `Min humidity`, `Wind`), boton rapido de metrica similar al panel rapido de capas, slider de opacidad y leyenda reutilizada para la metrica activa. La metrica seleccionada afecta siempre a puntos y leyenda; el boton `Heatmap` solo anade o quita la capa de densidad. No persistir aun esas preferencias porque la ruta es experimental; si se promociona a produccion, `heatmap` activo/inactivo, metrica y opacidad deben persistirse por `device_id` en `devices.json` con saneado backend y defaults compatibles.
...
- Versionado HA: `config.yaml`, labels Docker, banner runtime y cache-busters de visores estan alineados en `0.2.119`.
...
- El repo GitHub deberia quedar privado salvo ventanas operativas para que HA detecte updates; auditoria real del 2026-06-24 indica que ahora mismo esta publico (`private=false`, `visibility=public`). El paquete GHCR debe quedar accesible para HA. `0.2.119/latest` esta publicado con digest multi-arch `sha256:d6220a7ce7b186b7c598cbadadcb2f11c3d3bf41de3b33c5272dfcf2d993fe95`; GHCR conserva tambien `0.2.118` y `0.2.117` como rollback remoto. Tras validar `0.2.119` en HA, volver a poner el repo privado y limpiar versiones remotas antiguas conservando solo la version vigente, `latest` y las entradas auxiliares del mismo push multi-arch.
