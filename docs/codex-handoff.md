...
- Estado actual: version `0.2.137`, ingress, sidebar, imagen preconstruida `ghcr.io/cginebrosa/rainmapperha`, opciones de schedule, Google Maps API key, AEMET API key, mapas, fuentes, publish, `maplibre_hover_zoom` y defaults HA de heatmap (`maplibre_heatmap_weight_curve`, `maplibre_heatmap_opacity`, `maplibre_heatmap_radius`, `maplibre_heatmap_intensity`). La webUI muestra la version runtime en el panel de estado, agrupa las tarjetas de status en filas explicitas, muestra estado separado por fuente (`Meteoclimatic`, `Meteocat`, `Wunderground`, `AEMET`) y los enlaces de visores incluyen cache-buster de version para evitar cargas obsoletas en HA. La validacion de `Run all`, logs en ingles y schedule en la instalacion real de Home Assistant es manual/reportada por el usuario; pendiente de confirmar automaticamente. `0.2.137` esta publicada en GHCR y validada/dada por buena en HA para confirmar que el reset de defaults heatmap recoge cambios de `config.yaml` tras reiniciar la app; `0.2.134` queda como rollback validado previo. `0.2.123` queda publicada y validada visualmente en la ruta heatmap experimental. `0.2.116` corrigio el rendimiento de Tomap tras el problema de `0.2.115`. En `0.2.113`, se valida en HA el ajuste de ayuda movil MapLibre y los badges de fuente en dos lineas para conservar visible el contador de estaciones. En `0.2.112`, se publica en GHCR la ayuda `?` de MapLibre y la documentacion HA actualizada. En `0.2.111`, se alinean las atribuciones Meteoclimatic/Wunderground al criterio de informacion elaborada por Rainmapper y queda validada/dada por buena en HA por el usuario. En `0.2.109`, AEMET pasa al Tomap/GeoJSON estandar de HA y se desactiva la ruta experimental publica.
...
- Estado actual: usa Python 3.11 slim. Version alineada con `rainmapper-app/config.yaml` en `0.2.137`; tambien copia `users.example.json` para inicializar usuarios si falta configuracion persistente.
...
- Versionado HA: `config.yaml`, labels Docker, banner runtime y cache-busters de visores estan alineados en `0.2.137`.
...
- Imagen Docker HA multi-arch preconstruida en GHCR desde `0.2.57`; el repo confirma `image: ghcr.io/cginebrosa/rainmapperha` y el script `scripts/build-push-ha-image.sh`. La instalacion rapida en HA, el progreso de Supervisor, la poca utilidad del cache de GitHub Actions y la limpieza local observada son validaciones manuales/reportadas por el usuario; pendientes de confirmar automaticamente. Desde `0.2.60`, el flujo normal documentado es publicar la imagen desde el Mac con `scripts/build-push-ha-image.sh` antes de subir el commit de version; GitHub Actions queda como fallback manual. `0.2.137` y `latest` se publicaron y validaron en HA el 2026-06-25 con digest multi-arch `sha256:539c879d2c7f9dfc282d671b71c627a858b48d59778e3195ec2d0254accee928`; GHCR remoto queda limpio con `0.2.137/latest` y cuatro auxiliares sin tag. `0.2.134` queda como rollback visual validado previo. `0.2.132` queda como rollback operativo anterior validado. `0.2.129` se conserva como rollback visual previo documentado; `0.2.130` y `0.2.131` no son buenos rollback visuales para el popup.
...
- Revisar arquitectura de permisos de funcionalidades antes de anadir muchos mas flags por usuario. Ahora `can_use_heatmap` y `can_use_layer_metrics` viven en `users.json` por pragmatismo, pero si la app evoluciona conviene mover los permisos base a un JSON separado de perfiles/tipos de usuario (`free/basic/pro/admin` u otros) y dejar overrides opcionales por usuario.
...
- El repo GitHub queda privado salvo ventanas operativas para que HA detecte updates. Estado actual tras validar `0.2.137`: repo privado (`private=true`, `visibility=private`, rama `inicial`) y GHCR limpio conservando solo `0.2.137/latest` con digest multi-arch `sha256:539c879d2c7f9dfc282d671b71c627a858b48d59778e3195ec2d0254accee928` y cuatro auxiliares sin tag del mismo push multi-arch.
