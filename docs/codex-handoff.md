...
- Estado actual: usa Python 3.11 slim. Version alineada con `rainmapper-app/config.yaml` en `0.2.129`; tambien copia `users.example.json` para inicializar usuarios si falta configuracion persistente.
...
- Versionado HA: `config.yaml`, labels Docker, banner runtime y cache-busters de visores estan alineados en `0.2.129`.
...
- El repo GitHub deberia quedar privado salvo ventanas operativas para que HA detecte updates; auditoria real del 2026-06-24 indica que ahora mismo esta publico (`private=false`, `visibility=public`). El paquete GHCR debe quedar accesible para HA. `0.2.129/latest` esta publicado con digest multi-arch `sha256:fecfd9b74a85fc76f1e5fe15b5246fdb947325901dfcdaa19602ab5e3335daee`; GHCR conserva tambien `0.2.126` como rollback remoto hasta validar la nueva version. Tras validar `0.2.129` en HA, volver a poner el repo privado y limpiar versiones remotas antiguas conservando solo la version vigente, `latest` y las entradas auxiliares del mismo push multi-arch.
