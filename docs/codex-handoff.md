...
- `rainmapper-app/config.yaml`, `rainmapper-app/Dockerfile` y cache-busters de Leaflet/MapLibre estan alineados en version `0.2.150`.
- La imagen HA publicada documentada para `0.2.150/latest` es `ghcr.io/cginebrosa/rainmapperha` con digest multi-arch `sha256:35e42628eeb0937ec800608e9251fa0ef8148d4f6a626aea52a13a341ba71c0f`; commit de release `ecf2ed8`; no debe cerrarse como buena porque el boton `Mushroom catalogs` devolvia 404 bajo HA ingress y no sembraba `/share/rainmapper/mushroom-data/`. El repo contiene fix local pendiente de bump/publicacion: rutas relativas ingress-safe y seeding de defaults al arrancar. `0.2.149` queda instalada y validada/dada por buena en HA para el fix de User Management.
...
La app de Home Assistant esta configurada para funcionar como servicio `serve`, con webUI por ingress/sidebar, schedule interno, ejecuciones manuales `update`, `maps` y `all`, publicacion en `/config/www`, visores Bokeh, Leaflet y MapLibre, metricas basicas de Wunderground y fichero manual para ignorar estaciones anomalas en los GeoJSON. El funcionamiento en la instalacion real de HA ha sido validado manualmente por el usuario hasta la version `0.2.113`; esa validacion no es reproducible solo desde el repositorio. `0.2.113` queda validada/dada por buena en HA con ajustes MapLibre de ayuda movil y badges de fuente en dos lineas. `0.2.114` queda publicada con viento normalizado y preservacion de backfill AEMET; `0.2.115` no debe considerarse buena por rendimiento/legibilidad; `0.2.116` corrigio el rendimiento de Tomap y el usuario reporto `Generate maps` en 1:59; `0.2.118` queda validada visualmente en HA por el usuario el 2026-06-24. `0.2.119` queda publicada con alineacion de subcolumnas en historial MapLibre, diagnostico/contadores AEMET 429 y botones `Update only` por fuente en WebUI. `0.2.120` inicio el experimento MapLibre heatmap en ruta publica separada. `0.2.124` promociono el heatmap al visor protegido solo para usuarios `admin`. `0.2.125` corrigio la ocultacion real de botones heatmap/metrica para usuarios no admin, anadio pagina WebUI `/log` sin auto-refresh, reforzo la cabecera sticky del historial de popups y uso escala dinamica min/max para metricas no lluvia. `0.2.126` trata valores ausentes como sin dato en MapLibre y Tomap. `0.2.127` corrige la persistencia backend de settings Heatmap por dispositivo admin. `0.2.132` queda publicada en GHCR, pusheada a GitHub y validada visualmente en HA por el usuario el 2026-06-25. `0.2.133` queda publicada en GHCR y validada en HA el 2026-06-25 con permisos MapLibre por usuario para Heatmap y selector de metricas. `0.2.134` queda publicada en GHCR, pusheada a GitHub y validada visualmente en HA el 2026-06-25; corrige el espaciado de la cabecera sticky del historial MapLibre que tapaba el titulo `Ultims 30 registres`. `0.2.135` queda publicada y parcialmente validada: el popup de estacion con lluvia mas cercana queda validado, pero no debe cerrarse como version buena por el bug de defaults/reset de heatmap. `0.2.136` queda publicada pero no debe cerrarse como buena porque el `config.js` protegido podia quedar cacheado y no recoger cambios de defaults HA tras reinicio. `0.2.137` queda publicada en GHCR, pusheada a GitHub, validada/dada por buena en HA el 2026-06-25 y cerrada operacionalmente; sirve `/protected/maplibre/config.js` con `Cache-Control: no-store, max-age=0` para que `Reset heatmap defaults` use los defaults actuales de HA tras reiniciar la app. `0.2.138` publico la primera capa MapLibre experimental `IDW` calculada en cliente para el viewport visible, con permiso `can_use_estimated_field`, settings por dispositivo, defaults/tunables en `config.yaml` y correccion opcional de temperatura por altitud. `0.2.139` intento limitar el azul de lluvia cero, pero no resolvio suficientemente el comportamiento en HA. `0.2.140` corrigio sincronizacion Heatmap/IDW, usa radio y celda IDW en km y evita que lluvia cero cree area visible por si sola. `0.2.143` movio IDW por encima de estaciones para que la opacidad pueda taparlas. `0.2.144` queda publicada en GHCR y pusheada a GitHub; muestra en Settings los valores efectivos de radio, celda y potencia `p` del IDW. `0.2.145` queda publicada en GHCR y pusheada a GitHub; optimiza el refresco IDW con cache por clave de calculo y evita recalcados duplicados. `0.2.146` queda validada/dada por buena en HA con la WebUI Users compacta tipo accordion. `0.2.147` queda publicada en GHCR y pusheada a GitHub con el Control Panel redisenado como dashboard compacto con tabs internos; el usuario reporta que parece funcionar bien. `0.2.148` queda publicada pero no debe cerrarse como buena porque el acordeon de User Management aparecia totalmente desplegado en HA. `0.2.149` queda publicada en GHCR, pusheada a GitHub, instalada y validada/dada por buena en HA para corregir el acordeon de Users. `0.2.150` queda publicada en GHCR y pusheada a GitHub con la primera UI WebUI de mantenimiento de catalogos de setas, pero no debe cerrarse como buena por 404 en HA ingress; el repo contiene fix local pendiente de bump/publicacion. `0.2.109` publico AEMET integrado en el visor protegido estandar; `0.2.110` publico la redaccion de atribuciones AEMET/Meteocat como informacion/datos elaborados por Rainmapper. `0.2.111` queda validada/dada por buena en HA con Meteoclimatic/Wunderground alineados al mismo criterio de informacion elaborada por Rainmapper.

El repositorio GitHub `cginebrosa/RainmapperHA` se hizo privado el 2026-06-22 para no exponer codigo, rutas ni logica de uso de datos, especialmente Wunderground. Importante: tras hacer el repo privado se comprobo que Home Assistant no detecta updates nuevos porque necesita leer metadata del repo (`repository.yaml`, `config.yaml`, changelog); al volver a ponerlo publico detecto `0.2.101`. El flujo operativo actual para actualizar HA es publicar imagen GHCR, subir commit de version, hacer el repo publico temporalmente para que HA detecte la version, actualizar/validar en HA y volver a privado. Estado actual: `0.2.150` esta publicada en GHCR y pusheada a GitHub, pero no debe cerrarse como buena por 404 en HA ingress en la primera WebUI de mantenimiento de catalogos de setas; `0.2.149` esta instalada y validada/dada por buena en HA para User Management; `0.2.148` no debe cerrarse como buena por el bug visual del acordeon, `0.2.147` fue reportada por el usuario como funcionando bien para el Control Panel redisenado y `0.2.146` queda validada/dada por buena para la WebUI Users redisenada. `0.2.150/latest` queda publicado con digest multi-arch `sha256:35e42628eeb0937ec800608e9251fa0ef8148d4f6a626aea52a13a341ba71c0f`, commit `ecf2ed8`. `0.2.149/latest` queda publicado con digest multi-arch `sha256:3a488f597e34d2caba2c30edc90f5426813eb0c19858e2dcd679b197abda474b`, commit `039e615`. `0.2.148/latest` quedo publicado con digest multi-arch `sha256:a2fcab2222519150bd20a3f9cbb1949736b03384e1c6b79f36ef50d79d28c821`, commit `48629ff`. `0.2.147/latest` quedo publicado con digest multi-arch `sha256:368c910b9a31fba587c1e1cbca0201395feeecca3bf9e8884f62ccc08a76feef`, commit `9ffecab`. `0.2.145/latest` quedo publicado con digest `sha256:d77df69b135df3caa73d5a8fa4fc0d40354a6e150dbbfac86ab9d3a64a2c80b2`, commit `58b1ff9`. `0.2.137/latest` quedo publicado con digest multi-arch `sha256:539c879d2c7f9dfc282d671b71c627a858b48d59778e3195ec2d0254accee928`, commit `1bcd681`, repo privado y GHCR remoto limpio tras esa validacion. `0.2.134` queda como rollback visual validado previo. `0.2.135` y `0.2.136` quedan como antecedentes no cerrados por bugs de defaults/reset heatmap. `0.2.132` queda como rollback operativo anterior validado. `0.2.131` y `0.2.130` no son buenos candidatos visuales para rollback del popup porque mantienen el recorte de cabecera; `0.2.129` queda como rollback visual previo documentado. Cloudflare se endurecio operacionalmente fuera del repo: redireccion HTTP->HTTPS, HSTS `max-age=2592000; includeSubDomains`, `nosniff` y Cloudflare Access delante de `router.nomentero.com`, `leaflet.nomentero.com` y `maplibre.nomentero.com`. La ruta principal `rainmap.nomentero.com/protected/maplibre/index.html` queda servida por HTTPS y los GeoJSON protegidos siguen devolviendo `401` sin sesion Rainmapper.
...
- Estado actual: version `0.2.150` publicada en GHCR y pusheada a GitHub, pero no validada como buena por el 404 de `Mushroom catalogs` bajo HA ingress. La app mantiene ingress, sidebar, imagen preconstruida `ghcr.io/cginebrosa/rainmapperha`, opciones de schedule, Google Maps API key, AEMET API key, mapas, fuentes, publish, `maplibre_hover_zoom`, defaults HA de heatmap y defaults/tunables HA de IDW (`maplibre_estimated_field_*`). La webUI muestra la version runtime en el panel de estado, un Control Panel compacto con tabs internos, estado separado por fuente (`Meteoclimatic`, `Meteocat`, `Wunderground`, `AEMET`), enlaces de visores con cache-buster de version, User Management compacto tipo accordion con panel expandido reorganizado y entrada hacia `Mushroom catalogs`. La validacion de `Run all`, logs en ingles y schedule en la instalacion real de Home Assistant es manual/reportada por el usuario; pendiente de confirmar automaticamente para `0.2.150`. `0.2.149` queda validada/dada por buena en HA para User Management; `0.2.148` no debe cerrarse como buena por el bug visual del acordeon totalmente desplegado; `0.2.147` fue reportada por el usuario como funcionando bien para el Control Panel redisenado, `0.2.146` queda validada/dada por buena para la WebUI Users redisenada y `0.2.137` sigue siendo el rollback cerrado previo al bloque IDW. `0.2.134` queda como rollback visual validado previo.
...
- Estado actual: usa Python 3.11 slim. Version alineada con `rainmapper-app/config.yaml` en `0.2.150`; tambien copia `users.example.json` para inicializar usuarios si falta configuracion persistente y `mushroom-data/` como defaults versionados para sembrar `/share/rainmapper/mushroom-data/`.
...
- Versionado HA: `config.yaml`, labels Docker, banner runtime y cache-busters de visores estan alineados en `0.2.150`.
...
- Imagen Docker HA multi-arch preconstruida en GHCR desde `0.2.57`; el repo confirma `image: ghcr.io/cginebrosa/rainmapperha` y el script `scripts/build-push-ha-image.sh`. La instalacion rapida en HA, el progreso de Supervisor, la poca utilidad del cache de GitHub Actions y la limpieza local observada son validaciones manuales/reportadas por el usuario; pendientes de confirmar automaticamente. Desde `0.2.60`, el flujo normal documentado es publicar la imagen desde el Mac con `scripts/build-push-ha-image.sh` antes de subir el commit de version; GitHub Actions queda como fallback manual. `0.2.150` y `latest` se publicaron en GHCR el 2026-06-27 con digest multi-arch `sha256:35e42628eeb0937ec800608e9251fa0ef8148d4f6a626aea52a13a341ba71c0f`; commit `ecf2ed8`; no cerrar como buena por 404 en HA ingress; fix local pendiente de bump/publicacion. `0.2.149` esta instalada y validada/dada por buena en HA para User Management. `0.2.148` no debe cerrarse como buena por el bug visual del acordeon. `0.2.147` fue reportada por el usuario como funcionando bien para el Control Panel redisenado. `0.2.146` queda validada/dada por buena para la WebUI Users redisenada. `0.2.137` sigue siendo rollback cerrado previo al bloque IDW, con digest `sha256:539c879d2c7f9dfc282d671b71c627a858b48d59778e3195ec2d0254accee928`. `0.2.134` queda como rollback visual validado previo. `0.2.132` queda como rollback operativo anterior validado. `0.2.129` se conserva como rollback visual previo documentado; `0.2.130` y `0.2.131` no son buenos rollback visuales para el popup.
...
- El repo GitHub queda privado salvo ventanas operativas para que HA detecte updates. Estado actual: `0.2.150` esta publicada/pusheada pero no debe cerrarse como buena por 404 en HA ingress en la WebUI de catalogos de setas; `0.2.149` esta instalada y validada/dada por buena en HA para User Management; `0.2.148` no debe cerrarse como buena por el bug visual del acordeon, `0.2.147` fue reportada como funcionando bien y `0.2.146` queda validada/dada por buena. Tras validar `0.2.137`, el repo quedo privado (`private=true`, `visibility=private`, rama `inicial`) y GHCR limpio conservando solo `0.2.137/latest` con digest multi-arch `sha256:539c879d2c7f9dfc282d671b71c627a858b48d59778e3195ec2d0254accee928` y cuatro auxiliares sin tag del mismo push multi-arch. Tras validar una version posterior, repetir limpieza remota.
...
Validado manualmente en HA hasta `0.2.146`: el visor MapLibre protegido restaura settings por dispositivo desde `devices.json`, guarda cambios solo al cerrar Settings tras modificar controles del panel, cambia mapa desde el boton rapido de layers sin persistir `map_style`, mantiene la separacion entre periodo visible y periodo preferido, permite guardar explicitamente `map_view` como vista predeterminada sin escribir continuamente al mover el mapa, soporta selector de idioma ES/EN/CA, muestra controles moviles compactos, la WebUI `Users` refresca/busca correctamente sin perder scroll, AEMET esta integrado en el visor protegido estandar, no duplica diarios por tipos mixtos en `local_date`, `source_status.json` incluye `stations`, el historial largo del popup MapLibre queda validado visualmente en HA, los permisos Heatmap/Metric selector por usuario funcionan correctamente, el titulo `Ultims 30 registres` ya no queda tapado al desplegar y hacer scroll en el popup, `Reset heatmap defaults` usa los defaults actuales de HA tras reiniciar la app, e `0.2.146` valida la WebUI Users compacta tipo accordion. `0.2.147` fue reportada por el usuario como funcionando bien para el Control Panel redisenado. `0.2.148` no debe cerrarse como buena por el bug visual del acordeon totalmente desplegado. `0.2.149` queda validada/dada por buena en HA para el acordeon de User Management. `0.2.150` esta publicada/pusheada pero no debe cerrarse como buena por el 404 de `Mushroom catalogs` bajo HA ingress; el fix local queda pendiente de bump/publicacion. `0.2.135` y `0.2.136` no quedan como buenas para defaults/reset heatmap. Cierre `0.2.137`: digest multi-arch `sha256:539c879d2c7f9dfc282d671b71c627a858b48d59778e3195ec2d0254accee928`, commit `1bcd681`, repo privado y GHCR remoto limpio. Siguiente paso recomendado: publicar un bump con el fix local y validar acceso a `Mushroom catalogs`, siembra persistente de defaults, guardado con backup, creacion de entradas, import/export/plantilla y que User Management siga comportandose como en `0.2.149`.
