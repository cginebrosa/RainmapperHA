...
- Estado actual: usa Python 3.11 slim. Version alineada con `rainmapper-app/config.yaml` en `0.2.145`; tambien copia `users.example.json` para inicializar usuarios si falta configuracion persistente.
...
- Versionado HA: `config.yaml`, labels Docker, banner runtime y cache-busters de visores estan alineados en `0.2.145`.
...
- Imagen Docker HA multi-arch preconstruida en GHCR desde `0.2.57`; el repo confirma `image: ghcr.io/cginebrosa/rainmapperha` y el script `scripts/build-push-ha-image.sh`. La instalacion rapida en HA, el progreso de Supervisor, la poca utilidad del cache de GitHub Actions y la limpieza local observada son validaciones manuales/reportadas por el usuario; pendientes de confirmar automaticamente. Desde `0.2.60`, el flujo normal documentado es publicar la imagen desde el Mac con `scripts/build-push-ha-image.sh` antes de subir el commit de version; GitHub Actions queda como fallback manual. `0.2.145` y `latest` se publicaron en GHCR el 2026-06-27 con digest multi-arch `sha256:d77df69b135df3caa73d5a8fa4fc0d40354a6e150dbbfac86ab9d3a64a2c80b2`, pendiente de validar en HA. `0.2.137` sigue siendo la ultima version validada/dada por buena antes del bloque IDW, con digest `sha256:539c879d2c7f9dfc282d671b71c627a858b48d59778e3195ec2d0254accee928`. `0.2.134` queda como rollback visual validado previo. `0.2.132` queda como rollback operativo anterior validado. `0.2.129` se conserva como rollback visual previo documentado; `0.2.130` y `0.2.131` no son buenos rollback visuales para el popup.
