...
Nota de continuidad 2026-06-26: `0.2.139` quedo publicada y pusheada, pero en HA el usuario reporto que el IDW seguia pintando demasiado azul y que los botones `Heatmap`/`IDW` no sincronizaban bien su estado. `0.2.140` queda publicada en GHCR con digest multi-arch `sha256:2c6ca5354097ba98f4370e66fd1fab3b0e41ab3495fa0b15be0ea35e8022b85e`, pendiente de validar en HA: los botones rapidos no persisten en `devices.json`, los checkboxes de Settings si persisten y son incompatibles entre Heatmap e IDW. La capa IDW pasa a usar radio de influencia fijo en km y tamano fisico de celda en km (`maplibre_estimated_field_radius_*_km`, `maplibre_estimated_field_grid_*_cell_km`) para lluvia, temperatura, humedad y viento; las metricas ausentes/no numericas no participan y las temperaturas negativas siguen siendo validas. Lluvia cero no debe crear area visible por si sola, aunque puede influir en el promedio cerca de lluvia real. Documentar valores recomendados cuando se validen en HA y estudiar un multiplicador configurable por zoom si los zooms bajos requieren controlar coste/calidad.
...
- Estado actual: usa Python 3.11 slim. Version alineada con `rainmapper-app/config.yaml` en `0.2.140`; tambien copia `users.example.json` para inicializar usuarios si falta configuracion persistente.
...
- Versionado HA: `config.yaml`, labels Docker, banner runtime y cache-busters de visores estan alineados en `0.2.140`.
...
- Imagen Docker HA multi-arch preconstruida en GHCR desde `0.2.57`; el repo confirma `image: ghcr.io/cginebrosa/rainmapperha` y el script `scripts/build-push-ha-image.sh`. La instalacion rapida en HA, el progreso de Supervisor, la poca utilidad del cache de GitHub Actions y la limpieza local observada son validaciones manuales/reportadas por el usuario; pendientes de confirmar automaticamente. Desde `0.2.60`, el flujo normal documentado es publicar la imagen desde el Mac con `scripts/build-push-ha-image.sh` antes de subir el commit de version; GitHub Actions queda como fallback manual. `0.2.140` y `latest` se publicaron en GHCR el 2026-06-26 con digest multi-arch `sha256:2c6ca5354097ba98f4370e66fd1fab3b0e41ab3495fa0b15be0ea35e8022b85e`, pendiente de validar en HA. `0.2.137` sigue siendo la ultima version validada/dada por buena, con digest `sha256:539c879d2c7f9dfc282d671b71c627a858b48d59778e3195ec2d0254accee928`. `0.2.134` queda como rollback visual validado previo. `0.2.132` queda como rollback operativo anterior validado. `0.2.129` se conserva como rollback visual previo documentado; `0.2.130` y `0.2.131` no son buenos rollback visuales para el popup.
