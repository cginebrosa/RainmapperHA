...
from const import _DATA_PATH, _MAPS_PATH, _last_number_rains, _minimum_rain_tomap, _max_threads
...
def create_empty_incremental():
    # Keep the empty schema aligned with the incremental files expected by the map builders.
    return pd.DataFrame(columns=INCREMENTAL_COLUMNS)


def read_incremental(data_dir: Path, name: str, nrows=None):
    # Match Rainmapper.py type normalization so rebuilt Tomap files are comparable.
...
def merge_dataframes(data_dir: Path, source01_df_param: pd.DataFrame, source02_df_param: pd.DataFrame):
    # Preserve the legacy merge behavior: empty sources are replaced by an empty
    # incremental-shaped dataframe so column order stays stable.
...
def create_filtered(df_to_filter_param: pd.DataFrame, base_date, days_backward, days_forward):
    # Period filters are relative to local "today", matching the legacy Tomap block.
...
def create_grouped(df_to_group_param: pd.DataFrame, minimum_rain_tomap):
    # Aggregate one row per station for the selected period.
...
def build_tomap(data_dir: Path, maps_dir: Path, last_rains_history, minimum_rain_tomap, max_threads):
    # Rebuild all Tomap period files from existing incrementals. This intentionally
    # does not fetch weather data or modify Data/*_incremental.csv.
    maps_dir.mkdir(parents=True, exist_ok=True)
    ...
    for days_backward, file_name, label in TOMAP_PERIODS:
        # The 90-day dataframe is already filtered and reused as the source for shorter periods.
...
def main():
    ...
    parser.add_argument('--data-dir', default=_DATA_PATH, help='Directory containing incremental CSV files.')
    parser.add_argument('--maps-dir', default=_MAPS_PATH, help='Directory where Tomap CSV files will be written.')
    parser.add_argument(
        '--last-rains-history',
        type=positive_int,
        default=_last_number_rains,
        help='Number of recent rain records to generate for station popups.',
    )
    parser.add_argument(
        '--minimum-rain-tomap',
        type=float,
        default=_minimum_rain_tomap,
        help='Minimum daily rain included in Tomap last-rains history.',
    )
    parser.add_argument(
        '--max-threads',
        type=positive_int,
        default=max(1, _max_threads),
        help='Number of worker threads used while filtering source histories.',
    )
    args = parser.parse_args()

    data_dir = Path(os.path.abspath(args.data_dir))
    maps_dir = Path(os.path.abspath(args.maps_dir))
    exit_code = build_tomap(
        data_dir=data_dir,
        maps_dir=maps_dir,
