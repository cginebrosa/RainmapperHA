...
class MushroomWorkerPackagingTests(unittest.TestCase):
    def test_worker_image_is_minimal_and_uses_python_311(self) -> None:
        ...
        self.assertIn("ARG RAINMAPPER_WORKER_VERSION=1.0.13", dockerfile)
    ...
    def test_local_worker_service_is_startable_and_stoppable_without_volume_deletion(self) -> None:
        ...
        self.assertIn(
            "image: rainmapper-worker:${RAINMAPPER_WORKER_VERSION:-1.0.13}",
            compose,
        )
        self.assertIn(
            "RAINMAPPER_WORKER_VERSION: ${RAINMAPPER_WORKER_VERSION:-1.0.13}",
            compose,
        )
